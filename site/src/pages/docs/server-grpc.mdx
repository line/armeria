...
`RequestContext.current().blockingTaskExecutor().submit`, or set
...
class MyGrpcServiceImpl extends MyGrpcServiceImplBase {
    ...
    // method will be executed by the blocking task executor that is returned
    // by ServiceRequestContext.blockingTaskExecutor():
    @Blocking // 👈
