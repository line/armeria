...
We've implemented [CREATE](./tutorial-blog-rest-step3) and [READ](./tutorial-blog-rest-step4) operations.
...
Let's start by [mapping the HTTP PUT method](/docs/server-annotated-service#mapping-http-service-methods) with our service method:
...
For updating a blog post, let's take a blog post ID (`id`) and new blog post information to update with. For [creating a blog post](./tutorial-blog-rest-step3), we've used Armeria's <type://RequestConverter> to convert a request body into a Java object. For a change, let's try using <type://@RequestObject> to convert a request body.
...
Next, at [Step 6. Implement DELETE](./tutorial-blog-rest-step6), you'll implement a DELETE operation to delete blog posts.
