...
class CustomRouteTest {
    ...
    static ServerExtension server = new ServerExtension() {
        ...
        protected void configure(ServerBuilder sb) throws Exception {
            sb.service(GrpcService.builder()
                                  .addService("foo", new TestService())
                                  .addService("bar", new TestService())
    ...
    @Test
    void unframedJsonRequestWithCustomPath() {
        final WebClient client = WebClient.of(server.httpUri());
        final HttpRequest request =
                HttpRequest.of(HttpMethod.POST, "/foo/EmptyCall", MediaType.JSON_UTF_8, "{}");
    ...
    @Test
    void unframedProtobufRequestWithCustomPath() throws InvalidProtocolBufferException {
        final WebClient client = WebClient.of(server.httpUri());
        final SimpleRequest simpleRequest = SimpleRequest.newBuilder().setFillUsername(true).build();
        final HttpRequest request =
                HttpRequest.of(HttpMethod.POST, "/bar/UnaryCall", MediaType.PROTOBUF,
