...
public interface HttpRequest extends Request, HttpMessage {
    ...
    static HttpRequest from(RequestHeaders headers,
        ...
        if (stage instanceof CompletableFuture) {
            return from(headers, (CompletableFuture<? extends StreamMessage<? extends HttpObject>>) stage);
        } else {
            final DeferredHttpRequest req = new DeferredHttpRequest(headers);
            req.delegateWhenComplete(stage);
            return req;
        }
    }
    ...
    static HttpRequest from(RequestHeaders headers,
        ...
        if (future.isDone()) {
            if (!future.isCompletedExceptionally()) {
                final StreamMessage<? extends HttpObject> publisher = future.getNow(null);
            ...
            try {
                ...
                return of(headers, StreamMessage.aborted(Exceptions.peel(cause)));
            }
        }
    ...
    static HttpRequest from(RequestHeaders headers,
        ...
        final DeferredHttpRequest req = new DeferredHttpRequest(headers, subscriberExecutor);
        req.delegateWhenComplete(stage);
        return req;
    }
