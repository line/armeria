...
object HelloServiceGrpcKt {
  ...
  val blockingLotsOfRepliesMethod: MethodDescriptor<Hello.HelloRequest, Hello.HelloReply>
    @JvmStatic
  ...
  class HelloServiceCoroutineStub @JvmOverloads constructor(
     ...
     * @return A flow that, when collected, emits the responses from the server.
     */
    fun blockingLotsOfReplies(request: Hello.HelloRequest): Flow<Hello.HelloReply> =
        serverStreamingRpc(
      channel,
  ...
  abstract class HelloServiceCoroutineImplBase(
     ...
     * example.grpc.hello.HelloService.BlockingLotsOfReplies.
     *
     * If creating or collecting the returned flow fails with a [StatusException], the RPC
     * will fail with the corresponding [Status].  If it fails with a
     * [java.util.concurrent.CancellationException], the RPC will fail with status
     ...
     * @param request The request from the client.
     */
    open fun blockingLotsOfReplies(request: Hello.HelloRequest): Flow<Hello.HelloReply> = throw
