...
1. In the `BlogServiceTest` class, add a test method to retrieve the first blog post with ID `0`.
  ```java filename=BlogServiceTest.jvaa
  @Test
  ...
  ```
2. Add annotations to configure orders to execute our test methods.
...
To check that our exception handler is working, let's try retrieving a blog post that does not exist.
...
1. Bind the exception handler to the service for the test server.
  ...
  ```
2. Add a test method to retrieve a blog post with an invalid ID, asserting an exception is thrown.
  ```java filename=BlogServiceTest.java
