...
- You can now create an <type://HttpResponse> from a <type://ResponseHeaders> and
  a stream of <type://HttpData>s using
...
- You'll no longer see a `NullPointerException` when retrying a gRPC request.
...
- Fixed an issue where a Spring lifecycle object is created unnecessarily, even when
  an Armeria server did not start. #3300
