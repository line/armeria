...
Let's receive blog post information through a request body. Armeria's [request converter]
(/docs/server-annotated-service#converting-an-http-request-to-a-java-object) converts request parameters in HTTP messages into Java objects for you. In the request converter, we define what keys of a JSON object to map with what properties of a Java object.
...
2. Create and return an HTTP response using Armeria's <type://HttpResponse> with the information of the post created.
  ...
  ```java filename=BlogService.java highlight=3,5
