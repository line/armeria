...
- Implementing [DecoratingHttpServiceFunction](type) and [DecoratingRpcServiceFunction](type)
- Extending [SimpleDecoratingHttpService](type) and [SimpleDecoratingRpcService](type)
- Extending [DecoratingService](type)

## Implementing `DecoratingHttpServiceFunction` and `DecoratingRpcServiceFunction`

[DecoratingHttpServiceFunction](type) and [DecoratingRpcServiceFunction](type) are functional interfaces that
...
[SimpleDecoratingHttpService](type) or [SimpleDecoratingRpcService](type) depending on whether you are
decorating an [HttpService](type) or an [RpcService](type):
...
response. You can do that as well, of course, using [DecoratingService](type):
...
[ServiceWithRoutes](type) is a special variant of service which allows a user to register multiple
routes for a single service. It has a method called [ServiceWithRoutes#routes()](type) which returns a `Set` of
[Route](typeplural) so that you do not have to specify path when registering your service:
...
However, decorating a [ServiceWithRoutes](type) can lead to a compilation error when you attempt to
register it without specifying a path explicitly, because a decorated service is not a
[ServiceWithRoutes](type) anymore but just a service:
...
A good real-world example of [ServiceWithRoutes](type) is [GrpcService](type).
...
If you want to decorate multiple [Service](typeplural) by path mapping or router matching, you can specify
...
Please refer to [DecoratingServiceBindingBuilder](type) for more information.
