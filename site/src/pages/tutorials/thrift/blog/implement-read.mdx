...
First, add an exception and an exception handler class for the exception to be thrown when blog post with the specified ID doesn't exist.
...
3. Add a service method in the `BlogServiceImpl` class to retrieve multiple posts.
  ...
  ```
4. Add a sorting based on the value of the `descending` parameter.
  ```java filename=BlogServiceImpl.java
...
2. Set up log messages:
    ...
    2. Add the logback.xml file in the `{project_root}/src/main/resources` folder.
      ...
      ```
3. Restart the server and run the client.
...
1. Call the `getBlogPost()` method with `1`, an ID for the second blog post.
  ```java filename=BlogClient.java
  void testRun() {
    createBlogPost("My first blog", "Yay");
    createBlogPost("Another blog post", "Creating a post via createBlogPost().");
...
2. Re-run the server and run the client.
  ...
  ```
3. Check what happens if you retrieve a blog post that does not exist.
  Change the argument to `100` and run the client again.
  ```java filename=BlogClient.java
  void testRun() {
    createBlogPost("My first blog", "Yay");
    createBlogPost("Another blog post", "Creating a post via createBlogPost().");
