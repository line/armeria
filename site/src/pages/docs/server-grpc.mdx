...
annotate the gRPC services or methods with a `@Blocking` annotation, wrap the entire service implementation in
...
// so all methods are executed by by ServiceRequestContext.current().blockingTaskExecutor().
@Blocking // 👈
