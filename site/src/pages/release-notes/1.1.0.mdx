...
- <type://RequestLog#serviceNam()> is now used as a span name if you use the default `HttpRequestParser`
  and `HttpResponseParser` with `BraveClient` and `BraveService` for recognizing a span easily. #3023
...
- <type://TimeoutScheduler> is now initialized exactly once. #3066
