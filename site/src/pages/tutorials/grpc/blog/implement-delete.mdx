...
2. Add Armeria's <type://GrpcServiceBuilder>'s blocking task executor to make all gRPC methods executed in the blocking task executor's thread pool.
  ...
  By default, service methods are executed on the event loop, without thread switching, for fully asynchronous implementation. To implement blocking logic, call `useBlockingTaskExecutor(true)`.
...
4. Now, while keeping the server running, run the client again, attempting to delete the post with the ID `0`, which does not exist any longer.
   ...
   This time, we can't tell if the blog post was deleted or if an error has occurred. (Although we anticipate an error to occur.) This is because we haven't added anything in the exception block.
  ```java filename=BlogClient.java
    catch(StatusRuntimeException statusException) { 
      // Handle exception
    }
  ```
   Suppose we have no time and just want to know if an error has occurred. One way we can check is using the decorator again. So, change the client instance from `client` to `cb` in the `deleteBlogPost()` method and run the client again.
