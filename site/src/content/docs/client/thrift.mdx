...
Note that if a serialization format is unspecified, [ThriftSerializationFormats#BINARY](type) is used by
default. You can also specify one of [ThriftSerializationFormats#BINARY](type),
[ThriftSerializationFormats#COMPACT](type), [ThriftSerializationFormats#JSON](type)
and [ThriftSerializationFormats#TEXT](type)
using [ThriftClientBuilder#serializationFormat(SerializationFormat)](type).
...
The example above introduces a new class called [ThriftFuture](type). It is a subtype of Java 8
CompletableFuture\_ that implements Thrift [AsyncMethodCallback]. Once passed as a callback of an asynchronous
Thrift call, [ThriftFuture](type) will complete itself when the reply is received or the call
...
to gather metrics. Please also refer to [ThriftClientBuilder](type) for more configuration options.
