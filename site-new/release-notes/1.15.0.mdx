...
- Preliminary GraphQL support in [DocService](type) 
...
- [Server](type) now registers two gauges about TLS certificate expiry, so you can get alerts before
...
- You can now use the `||` (logical OR) operator in parameter and header predicate expressions. #4116 #4138
  ...
  using [@MatchesParam](type) and [@MatchesHeader](type).
...
- You can now enable [gRPC health check protocol](https://github.com/grpc/grpc/blob/master/src/proto/grpc/health/v1/health.proto)
  with [GrpcHealthCheckService](type) #3146 #3963
  ```java
  Server
    .builder()
    .service(
    ...
    .build()
  ```
- [DocService](type) now supports [GraphQlService](type) with CodeMirror auto-completion. #3706 #4023
- You can now configure the [EndpointGroup](type) implementations based on [DynamicEndpointGroup](type)
...
- You can now specify a [SuccessFunction](type) when constructing a client or server, and let all decorators
  ...
  WebClient
    ...
    .build();
  ```
- You can now specify different sampling ratio (or [Sampler](type)) for successful and failed requests
  when using [LoggingClient](type) and [LoggingService](type) #3666 #4101
  ```java
  LoggingService
    .builder()
    ...
    .newDecorator()
  ```
- You can now customize how [LoggingClient](type) and [LoggingService](type) determines log level
  more flexibly. #3972
  ```java
  LoggingService
    .builder()
    ...
    .newDecorator()    
  ```
- You can now specify a customizer that customizes a [ClientRequestContext](type) when building a client. #4075 #4082
  ```java
  WebClient client =
    WebClient
      .builder()
  ...
  WebClient
    ...
    .build();
  ```
- You can now write the content of [StreamMessage](type) into a file using
  [StreamMessage#writeTo(Function,Path,OpenOption)](type) #4048 #4130
  ```java
  ...
  ```
- You can now convert a [StreamMessage](type) into an `InputStream` using
  [StreamMessage#toInputStream(Function)](type) #4059
  ```java
  ...
  ```
- You can now use [StreamMessage#mapParallel(Function,int)](type) to modify a stream using an async operation
  with a configurable concurrency limit. #4031
- You can now decode a [StreamMessage](type) into another using [StreamMessage#decode(StreamDecoder)](type)
  which was previously possible only for [HttpMessage](type). #4147 #4148 #4152
- You can now give Armeria a performance optimization hint by specifying if your service is unary, 
  request-streaming, response-streaming or bidi-streaming by implementing [HttpService#exchangeType()](type) #3956
  ```java
...
- [GrpcService](type) now logs [RequestOnlyLog#requestContent()](type) for invalid requests. #4128
- [DocService](type) doesn't crash the whole Armeria service anymore when `.proto` files have comments
  on service options or extensions. #4123 #4127
- [GraphQlService](type) doesn't throw a `ClassCastException` anymore when `query` or `operationName` are not
  a string. #4104 #4168  
- [FileService](type) doesn't choose a `.br` file for generating decompressed content anymore when Brotli
  is not available. #4119
- You no longer see an [HttpResponseException](type) or [HttpStatusException](type) from built-in services. #4056 #4117
  - Thanks to this fix, you can now mutate a redirect response from [FileService](type) in your decorator
    using [HttpResponse#mapHeaders(Function)](type)
- [RampingUpEndpointWeightSelector](type) doesn't raise a `NoSuchElementException` anymore when endpoints
  are replaced completely. #3776 #4102

## 🏚️ Deprecations

- `samplingRate` property of [@LoggingDecorator](type) has been deprecated in favor of `successSamplingRate`
  and `failureSamplingRate`.
- The `successFunction()` builder methods that require a `BiPredicate` has been deprecated in favor of
  [SuccessFunction](type) that's specified when constructing a client or server.
...
- The experimental `HttpDecoder` API has been replaced with [StreamDecoder](type) #4147 #4152
