...
- <type://HttpData> now extends `AutoCloseable` and provides various methods useful when using
  ...
  - Note that this feature is only for the advanced users who wants to reduce the number of memory copies
...
- Added `final` to the various classes and methods so that a user doesn't extend them beyond the devs'
  intention. #2927
- `com.linecorp.amreria.common.util.NonNullByDefault` and `UnstableApi` have been moved to
