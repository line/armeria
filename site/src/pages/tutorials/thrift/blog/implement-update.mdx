...
1. In the `testRun()` method, call the `updateBlogPost()` method to test run the update method we just implemented.
  Let's test updating the second blog post with ID `1`.
  ```java filename=BlogClient.java
...
2. [Re-run the server](/tutorials/thrift/blog/run-server-and-client#2-run-the-server) and run the client.
  ...
  Let's dig into the logs to see if it worked as expected.
  ...
  This time, search for "method=updateBlogPost" in the subsequent logs.
  ...
  Again, search for "method=getBlogPost" in the subsequent logs.
  Check that you see the title of the blog post is updated to `New title`.
  ```bash
  [armeria-common-worker-nio-2-2] DEBUG c.l.a.c.logging.LoggingRpcClient - ... { ... , content=DefaultRpcRequest{
  serviceType=BlogService$Iface, 
  serviceName=example.armeria.blog.thrift.BlogService$Iface, 
