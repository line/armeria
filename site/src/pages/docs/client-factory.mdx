...
You can configure the TLS key and certificates
for [mutual TLS (mTLS)](https://www.cloudflare.com/ko-kr/learning/access-management/what-is-mutual-tls/)
using <type://ClientFactoryBuilder#tls(InputStream,InputStream)> .
Additionally, you can specify various options such as a different CA certificate chain or enforce certain 
cipher suites with <type://ClientFactoryBuilder#tlsCustomizer(Consumer)> .
...
ClientFactory builder =
  ClientFactory
    .builder()
    .tls(getClass().getResourceAsStream(resourceRoot + "custom.crt"),
