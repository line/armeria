...
<RequiredDependencies
  ...
  dependencies={[
    { groupId: 'com.linecorp.armeria', artifactId: 'armeria-saml' },
  ]}
/>
...
Finally, you need to create your [SamlServiceProvider](type) with a [SamlServiceProviderBuilder](type), and
attach it to your [Server](type).
...
`armeria-saml` provides [SamlSingleSignOnHandler](type) to handle the response from an identity provider.
...
The following example shows a simple implementation of the [SamlSingleSignOnHandler](type). In this example,
...
[Authorizer](type) can identify an authenticated session with a `Cookie` header in the following requests,
...
that you write your own [SamlSingleSignOnHandler](type) according to your authentication system.
