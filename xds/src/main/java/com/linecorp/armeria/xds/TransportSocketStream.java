    ...
    protected Subscription onStart(SnapshotWatcher<TransportSocketSnapshot> watcher) {
        ...
        if (commonTlsContext.hasValidationContext()) {
            ...
            validationStream = secretStream.switchMap(
                    snapshot -> new CertificateValidationContextStream(context, snapshot));
            ...
            final SecretStream secretStream = new SecretStream(sdsConfig, configSource, context);
            validationStream = secretStream.switchMap(snapshot -> new CertificateValidationContextStream(
                    context, snapshot, combined.getDefaultValidationContext()));
        } else {
        ...
        if (!commonTlsContext.getTlsCertificatesList().isEmpty()) {
            ...
            final SecretStream secretStream = new SecretStream(secret, context);
            tlsCertStream = secretStream.switchMap(snapshot -> new TlsCertificateStream(context, snapshot));
