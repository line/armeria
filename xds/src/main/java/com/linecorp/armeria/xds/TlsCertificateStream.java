...
final class TlsCertificateStream extends RefCountedStream<TlsCertificateSnapshot> {
    ...
    TlsCertificateStream(SubscriptionContext context, SecretSnapshot secretSnapshot) {
        this.context = context;
        resource = secretSnapshot.xdsResource();
    }
                ...
                SnapshotStream.combineNLatest(childBuilders.build())
                              .map(combined -> {
                                  ...
                                  if (privKeyBytes.isPresent() && certChainBytes.isPresent()) {
                                      final Optional<ByteString> passwordBytes = combined.get(2);
                                      if (passwordBytes.isPresent()) {
                                          final String password = passwordBytes.get().toStringUtf8();
                                          tlsKeyPair = TlsKeyPair.of(privKeyBytes.get().newInput(), password,
                                                                     certChainBytes.get().newInput());
                                      } else {
                                          tlsKeyPair = TlsKeyPair.of(privKeyBytes.get().newInput(),
                                                                     certChainBytes.get().newInput());
                                      }
                                  } else {
                                      tlsKeyPair = null;
