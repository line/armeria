...
Previously, we created and tested creating a blog post. In this step, we'll implement a read operation and make a call 
to read blog posts. We'll write two service methods for reading a single post and multiple posts each.

One major difference to the earlier step is the client instance. Instead of using the client instance we created earlier, we'll use another one. Armeria offers various options to create client instances.
...
This time, we'll build a client differently. We do already have a client instance we created for creating a blog post, created with the basic client creator, <type://Clients#newClient>. Now, we'll use  <type://Clients#builder>.
...
1. Add another <type://BlogServiceBlockingStub> in the `ClientMain` class with a different name.
  ...
  ```
2. Create a client instance with <type://Clients#builder>.
  ```java filename=BlogClient.java
  class BlogClient {
  ...
  ```
3. Add a method each for retrieving posts. Note, this time we're using `cb`, the client instance built with <type://Clients.builder>.
...
Since we haven't added any log messages, we can't tell if we've successfully retrieved blog posts or not. To check the result, we'll add in an interceptor as the next step. Not only can we see the blog posts retrieved, we can obtain IDs to retrieve a post later on.

## 4. Add interceptor

We want to retrieve a single blog post, but we don't have the post ID information. (Although we very much know how the IDs are generated in this tutorial.) Here, what we'll do is intercept the response from the server to get the ID.

1. Add a decorator to the client instance, `cb`, to intercept response.
  ```java filename=BlogClient.java
...
Here, we've implemented service methods and client methods to retrieve blog posts. Plus, we added an interceptor. See [Decorating a client](/docs/client-decorator/) for more information. Next, we'll implement a method for [updating a blog post](/tutorials/grpc/blog/implement-update) and add an exception handler.
