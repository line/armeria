...
public class EventLoopStreamMessage<T> implements StreamMessageAndWriter<T>, Subscription, StreamCallbacks<T> {
    ...
    public final void subscribe(Subscriber<? super T> subscriber, EventExecutor executor,
        ...
        if (eventLoop.inEventLoop()) {
            ...
            eventLoop.execute(() -> subscribe0(subscriber, executor, options));
        }
    }
    ...
    private void subscribe0(Subscriber<? super T> subscriber, EventExecutor executor,
        ...
        notifyOnSubscribe(this, subscriber, executor);
        tryProcessEventQueue();
    }
    ...
    private void notifyOnSubscribe(Subscription subscription, Subscriber<?> subscriber,
        ...
        if (!executor.inEventLoop()) {
            executor.execute(() -> notifyOnSubscribe(subscription, subscriber, executor));
            return;
        }
        try {
            subscriber.onSubscribe(subscription);
