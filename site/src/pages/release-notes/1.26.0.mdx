...
- You can now add an `EventLoopGroup` to a server, a virtual host and a service to run a service with it.
  #4099 #5233
  ```java
  final EventLoopGroup serviceEventLoopGroup =
    EventLoopGroups.newEventLoopGroup(8, "service-worker");
  Server.builder()
        .eventLoopGroup(serviceEventLoopGroup)
        .service(myService) // myService is run with serviceEventLoopGroup.
        ...
        .build();
  ```
- You can now set services/decorators under a context path fluently. #3591 #4802 #5037 #5157
  ```java
  Server.builder()
        .contextPath("/v1", "/v2")
        ...
        .build();
  ```
- You can now easily propagate an Armeria <type://RequestContext> in Kotlin using
  ...
  ```
- You can now produce the last value for a <type://StreamMessage> when it completes exceptionally using
  ...
  ```
- You can now disable HTTP/2 protocol when sending HTTP or HTTPS requests. #5168
  ```java
  ClientFactory
    .builder()
    .preferHttp1(true)
    .build();
  ```
- You can now monitor client certificates with the following metrics. #5155
...
- You can now add a hook that is invoked when a <type://RequestContext> is pushed and popped. #4991 #5107
  ...
  Server
    ...
    .build();
  ```
- You can now report a failure to a circuit breaking when the total duration exceeds a certain threshold.
  #4782 #4827
  ```java
  final CircuitBreakerRuleWithContent<HttpResponse> rule =
        CircuitBreakerRuleWithContent
                ...
                .thenFailure();
  ```
- You can now protect your server against [DDoS caused by RST floods](https://www.securityweek.com/rapid-reset-zero-day-exploited-to-launch-largest-ddos-attacks-in-history/). #5232
  ```java
  Server
    .builder()
...
- TBW about gRPC service builder#5046
