...
This time, we'll use a different client instance. Instead of using the client instance created with the <type://Clients>, we'll use <type://GrpcClients>.

1. Add another <type://BlogServiceBlockingStub> in the `BlogClient` class with a different name.
  ...
  ```
2. Add a new client instance with <type://GrpcClients>.
  ```java filename=BlogClient.java
  class BlogClient {
    public static void main(String[] args) throws Exception {
      ...
      // add this one here
      cb = GrpcClients.builder("gproto+http://127.0.0.1:8080/")
                      .build(BlogServiceBlockingStub.class);
    }
  }
  ```
3. Add a method each for retrieving posts. Note, this time we're using `cb`, the client instance created with <type://GrpcClients>.
...
We want to retrieve a single blog post, but we don't have the post ID information. (Although we very much know how the IDs are generated in this tutorial.) Here, what we'll decorate the client the response from the server to get the ID.
...
2. Set up to log messages:
    ...
    - Add the logback.xml file in {project_root}/src/main/resources folder.
      ...
      ```
3. Stop the server and run both the server and the client. If we succeed,we'll see an output like the following.
  ```bash
