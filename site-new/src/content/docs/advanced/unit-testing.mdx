...
- [ClientRequestContext](type) or [ServiceRequestContext](type)
- [HttpRequest](type) or [RpcRequest](type)

[ClientRequestContext](type) or [ServiceRequestContext](type) is a more complex object with many properties than
[HttpRequest](type) or [RpcRequest](type), and thus Armeria provides the API dedicated to building a fake context
object easily:
...
Although the fake context returned by [ClientRequestContext#of(HttpRequest)](type) and
[ServiceRequestContext#of(HttpRequest)](type) will provide sensible defaults,
