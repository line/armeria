...
You can use [ServerListener](type) to register the information of Armeria servers when they start so that
...
Armeria provides 2 [ServerListener](type) implementations for service registration:

- [ZooKeeperUpdatingListener](type) which registers to [ZooKeeper](https://zookeeper.apache.org/)
  as an ephemeral node.
- [EurekaUpdatingListener](type) which registers to [Eureka](https://github.com/Netflix/eureka/).
...
Then, use [ZooKeeperUpdatingListener](type) and [ZooKeeperRegistrationSpec](type) to register your server
...
The [ZooKeeperRegistrationSpec](type) is used to convert the information of your server to a binary
representation. The [ZooKeeperUpdatingListener](type) registers the binary representation to the specified
...
[ZooKeeperRegistrationSpec#curator(String)](type) uses the format of
...
You can use [ZooKeeperRegistrationSpec#serverSets()](type) that applies the format of
[Finagle ServerSets](https://twitter.github.io/finagle/docs/com/twitter/serverset.html).
If you want to use your own format, you can implement the [ZooKeeperRegistrationSpec](type).
...
Then, use [EurekaUpdatingListener](type) to register your server to the Eureka:
...
of the [Server](type), use [EurekaUpdatingListenerBuilder](type):
...
For more information, please refer to the API documentation of the
[com.linecorp.armeria.server.eureka](https://javadoc.io/doc/com.linecorp.armeria/armeria-javadoc/latest/com/linecorp/armeria/server/eureka/package-summary.html) package.
...
## [Consul](https://www.consul.io)-based service registration with `ConsulUpdatingListener`
...
Then, use [ConsulUpdatingListener](type) to register your server to the Consul:
...
If you want to specify an [HTTP check](https://www.consul.io/api/agent/check#http) or
[tags](https://www.consul.io/api-docs/agent/service#tags) to the [Server](type),
use [ConsulUpdatingListenerBuilder](type):
...
For more information, please refer to the API documentation of the
[com.linecorp.armeria.server.consul](https://javadoc.io/doc/com.linecorp.armeria/armeria-javadoc/latest/com/linecorp/armeria/client/consul/package-summary.html) package.

:::tip

See [Consul service discovery](/docs/client/service-discovery#consul-based-service-discovery-with-consulendpointgroup).
