...
Waiting for the chunk to be written is to avoid loading data into memory when the client is not ready to receive. This is called **back pressure**.
...
To send a file with back pressure, use [HttpFile](type), which loads and sends file chunks one by one to the client with back pressure.
...
To send large data other than files such as database, you need to implement back pressure yourself. Let's start off with implementing a minimal [HttpFile](type).

Prepare to send a streaming response with [HttpResponseWriter](type) returned by [HttpResponse#streaming()](type).
...
With the code above, the server would encounter `OutOfMemoryError`. We still need to take care of preventing loading data chunks into memory before a chunk is sent to the client. To solve the problem, implement back pressure with [StreamWriter#whenConsumed()](type):
...
So far, we have implemented a simple version of [HttpFile](type). Now, we can implement a streaming response with back pressure for any type of source (e.g. database) by simply changing the `produceChunk()` method to fetch data from the source.
...
return the response using [HttpResponse#of(ResponseHeaders,Publisher)](type):
