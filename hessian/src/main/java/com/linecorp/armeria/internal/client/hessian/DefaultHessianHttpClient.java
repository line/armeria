...
final class DefaultHessianHttpClient extends UserClient<RpcRequest, RpcResponse> implements HessianClient {

    DefaultHessianHttpClient(ClientBuilderParams params, RpcClient delegate, MeterRegistry meterRegistry) {
        super(params, delegate, meterRegistry, RpcResponse::from, (ctx, cause) -> RpcResponse.ofFailure(cause));
    }
    ...
    private RpcResponse execute0(String path, Class<?> serviceType, @Nullable String serviceName, String method,
                                 Object[] args) {
        ...
        // A hessian path is always good to cache as it cannot have non-fixed parameters.
        pathAndQuery.storeInCache(path);

        final RpcRequest call = RpcRequest.of(serviceType, method, args);
        return execute(scheme().sessionProtocol(), HttpMethod.POST, pathAndQuery.path(), null, serviceName,
                       call);
    }
