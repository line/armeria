...
- <type://DocService> now exposes a JSON specification via the `/schemas.json` endpoint. #4518
...
- gRPC HTTP/JSON transcoding now doesn't throw an exception when used with nested messages. #4631 #4641
...
- Warning on duplicate parameters in <type://DocService> are no longer visible when annotations are added to Kotlin data classes. #4645
- gRPC HTTP/JSON transcoding now correctly interprets the `google.protobuf.FieldMask` type as a string. #4633 #4640
- gRPC metadata is now exposed via <type://StatusRuntimeException> in `RequestLog.responseCause()`. #4635 #4636
- Square brackets "`[]`" are correctly encoded when specified in query or path parameters. #4607 #4615
