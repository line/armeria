...
1. Create a main class for our server. You can see the full version of the file [here](https://github.com/line/armeria-examples/blob/master/tutorials/rest-api-annotated-service/src/main/java/example/armeria/server/blog/Main.java).
...
We've only tested running the service on a server. This time, let's try calling the service. To test, we'll write a simple client using Armeria's <type://Clients#newClient(Scheme,EndpointGroup,Class)> and the Java code generated from [Defining a service](/tutorials/grpc/blog/define-service).
...
2. Create a client using Armeria's <type://Clients>. We're running the server locally and have set the port to 8080 in our server code. Let's access the server at http://127.0.0.1:8080.
  ...
  final class BlogClient {
    ...
    public static void main(String[] args) throws Exception {
      client = Clients.newClient("gproto+http://127.0.0.1:8080/",
...
The message says "UNIMPLEMENTED". This is because we are yet to implement the service method to create a blog post. One thing we've checked is that our service does return something to a client call.
