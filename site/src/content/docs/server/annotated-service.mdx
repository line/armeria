...
the glob pattern in your service method by annotating a parameter with [@Param](type) as follows.
Please refer to [Parameter injection](#parameter-injection) for more information about [@Param](type).
...
to map more than one HTTP method or path to your service method? You can use [@Path](type) annotations to
...
you can use [@StatusCode](type) annotation as follows.
...
Note that you can omit the value of [@Param](type) if you compiled your code with `-parameters` javac
...
[@Param](type) annotation. If your `Enum` type can be handled in a case-insensitive way, Armeria
...
When the value of [@Param](type) annotation is not shown in the path pattern, it will be handled as a
...
you can use [@Default](type) annotation, `@Nullable` annotation or `Optional<?>` class, like demonstrated
...
`@Nullable` annotation can be from _any_ package as long as its simple name is `Nullable`.
...
or `Set<?>`, e.g. `/hello1?number=1&number=2&number=3`. You can use [@Default](type) annotation
...
If you specify query delimiter while building an annotated service, or a [@Delimiter](type) annotation with a
...
no [@Param](type) value appears in the path pattern, Armeria will aggregate the received request and
...
Armeria also provides [@Header](type) annotation to inject an HTTP header value into a parameter.
The parameter annotated with [@Header](type) can also be specified as one of the built-in types as follows.
[@Default](type) and `Optional<?>` are also supported. [@Header](type) annotation also supports
...
Note that you can omit the value of [@Header](type) if you compiled your code with `-parameters` javac
...
`content_length` will be converted to `content-length` as the value of [@Header](type).
...
Armeria also provides the [@Attribute](type) annotation to inject a
custom attribute value of a [RequestContext](type) into the annotated
parameter. The [@Attribute](type) annotation has two attributes:
...
Note that if no attribute for the specified `AttributeKey` is found in [RequestContext](type), `IllegalArgumentException` will occur at runtime.
...
- [ServiceRequestContext](type) (or [RequestContext](type))
- [RequestHeaders](type) (or [HttpHeaders](type))
- [HttpRequest](type) (or [Request](type))
- [AggregatedHttpRequest](type)
- [QueryParams](type)
- [Cookies](type)
...
Armeria provides [@ExceptionHandler](type) annotation to transform an exception into a response.
You can write your own exception handler by implementing [ExceptionHandlerFunction](type) interface and
annotate your service object or method with [@ExceptionHandler](type) annotation. Here is an example of
an exception handler. If your exception handler is not able to handle a given exception, you can call
[ExceptionHandlerFunction#fallthrough()](type) to pass the exception to the next exception handler.
...
the default exception handler. It handles `IllegalArgumentException`, [HttpStatusException](type) and
[HttpResponseException](type) by default. `IllegalArgumentException` would be converted into
...
a Java object automatically. Armeria provides [@RequestConverter](type) and [@RequestObject](type)
annotations so that such conversion can be done conveniently.
You can write your own request converter by implementing [RequestConverterFunction](type) as follows.
Similar to the exception handler, you can call [RequestConverterFunction#fallthrough()](type) when your request
...
by the converters you registered with [@RequestConverter](type) annotation.
Note that [@RequestConverter](type) annotation can be specified on a class, a method or a parameter
...
[JacksonRequestConverterFunction](type) for a JSON document, [StringRequestConverterFunction](type)
for a string and [ByteArrayRequestConverterFunction](type) for binary data. They will be applied
after your request converters, so you don't have to specify any [@RequestConverter](type) annotations:
...
we should put [@Param](type), [@Header](type), [@RequestObject](type) annotations on any of the following elements:
...
The usage of [@Param](type) or [@Header](type) annotations on Java object elements is much like
using them on the parameters of a service method because even you can use [@Default](type) and
[@RequestObject](type) annotations defined there.
...
response converters, except for [HttpResponse](type) and [AggregatedHttpResponse](type) which are already
in a form of response message. You can also write your own response converter by implementing
[ResponseConverterFunction](type) as follows. Also similar to [RequestConverterFunction](type),
you can call `ResponseConverterFunction.fallthrough()` when your response converter is not able to
convert the result to an [HttpResponse](type).
...
Even if you do not specify any [@ResponseConverter](type) annotation, the response object can be converted into
an [HttpResponse](type) by one of the following response converters which performs the conversion based on
the negotiated media type and the type of the object.

- [JacksonResponseConverterFunction](type)
...
- [StringResponseConverterFunction](type)
...
- [ByteArrayResponseConverterFunction](type)
  - converts an object to a byte array. Only [HttpData](type) and `byte[]` will be handled
    even if the negotiated media type is `application/binary` or `application/octet-stream`.
    If there is no media type negotiated, [HttpData](type) and `byte[]` object will be written as a binary
...
If you specify the [@Blocking](type) annotation to the method, the method will be executed by
the blocking task executor that is returned by [ServiceRequestContext#blockingTaskExecutor()](type):
...
You can apply the [@Blocking](type) annotation at the class level so all methods are executed by the
...
You can also submit a task to the [ServiceRequestContext#blockingTaskExecutor()](type) or any `Executor`
instead of using the [@Blocking](type) annotation:
...
You can specify converters and exception handlers using [ServerBuilder](type), without using the annotations
...
in a single class and add it to your [ServerBuilder](type) at once, e.g.
...
In the earlier examples, the annotated service methods only return [HttpResponse](type), however there are
more response types which can be used in the annotated service.

- [HttpResponse](type) and [AggregatedHttpResponse](type)
  - It will be sent to the client without any modification. If an exception is raised while the response is
    ...
    the exception handler can send an [HttpResponse](type) instead.

- [ResponseEntity](type)
  - It contains the [HttpHeaders](type) and the object which can be converted into HTTP response body by
...
- Reactive Streams [Publisher]
  - All objects which are produced by the publisher will be collected, then the collected ones will be
    converted to an [HttpResponse](type) by response converters. If a single object is produced, it will be
...
- `CompletionStage` and `CompletableFuture`
  - An object which is generated by the `CompletionStage` will be converted to an [HttpResponse](type)
...
Every [HttpService](type) can be wrapped by another [HttpService](type) in Armeria (Refer to
[Decorating a service](/docs/server/decorator) for more information).
Simply, you can write your own decorator by
implementing [DecoratingHttpServiceFunction](type) interface as follows.
...
Then, annotate your class or method with a [@Decorator](type) annotation. In the following example,
...
As you read earlier, you can write your own decorator with [DecoratingHttpServiceFunction](type) interface.
...
decorator annotation which applies [LoggingService](type) to an annotated service.

:::tip

This example is actually just a copy of what Armeria provides out of the box. In reality,
you could just use [@LoggingDecorator](type), without writing your own one.
...
You can see [@DecoratorFactory](type) annotation at the first line of the example. It specifies
a factory class which implements [DecoratorFactoryFunction](type) interface. The factory will create
an instance of [LoggingService](type) with parameters which you specified on the class or method like below.
...
The first rule is as explained before. However, if your own decorator annotations and [@Decorator](type)
annotations are specified in a mixed order like below, you need to clearly specify their order using [@Decorator#order()](type)
attribute of the annotation. In the following example, you cannot make sure in what order they decorate
the service because Java collects repeatable annotations like [@Decorator](type) into a single container
annotation like [@Decorators](type) so it does not know the specified order between [@Decorator](type)
and [@LoggingDecorator](type).
...
If you built a custom decorator annotation like [@LoggingDecorator](type), it is recommended to
...
Armeria provides [@Produces](type) and [@Consumes](type) annotations to support media type
...
negotiated [@Produces](type) from [ServiceRequestContext#negotiatedResponseMediaType()](type) method and
...
In this case, you can adjust the order of the methods with [@Order](type) annotation. The default value of
[@Order](type) annotation is `0`. If you set the value less than `0`, the method is used earlier than
...
You can annotate them with [@Consumes](type) annotation.
...
specified with [@Consumes](type), the client would get an HTTP status code of 415 which means
...
Armeria provides pre-defined annotations such as [@ConsumesJson](type), [@ConsumesText](type),
[@ConsumesBinary](type) and [@ConsumesOctetStream](type) which are aliases for
...
Also, [@ProducesJson](type), [@ProducesText](type), [@ProducesBinary](type) and [@ProducesOctetStream](type)
are provided in the same manner.

If there is no annotation that meets your need, you can define your own annotations for [@Consumes](type)
and [@Produces](type) as follows. Specifying your own annotations is recommended because writing a media type
...
[@AdditionalHeader](type) for HTTP headers and [@AdditionalTrailer](type) for HTTP trailers.
...
The [@AdditionalHeader](type) or [@AdditionalTrailer](type) specified at the method level takes precedence over
...
You can override it by annotating a class or method with [@ServiceName](type) like the following:
...
You can also use [RequestLogAccess#whenComplete()](type) to access [RequestOnlyLog#name()](type).
