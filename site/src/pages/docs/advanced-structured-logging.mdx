...
Each type of connection timing properties are represented as <type://ClientConnectionTimingsType>.
Each property records a start time and duration if available.
...
+------------------------------------------+-------------------------------------------------------------------+
| `dnsResolution.startTimeMicros`          | when the client started to resolve a domain name, in microseconds |
|                                          | since the epoch (01-Jan-1970 00:00:00 UTC), `-1` if DNS lookup    |
|                                          | did not occur                                                     |
+------------------------------------------+-------------------------------------------------------------------+
| `dnsResolution.durationNanos`            | the duration took to resolve a domain name, `-1` if DNS lookup    |
|                                          | did not occur                                                     |
+------------------------------------------+-------------------------------------------------------------------+
| `socketConnect.startTimeMicros`          | when the client started to connect to a remote peer, in           |
|                                          | microseconds since the epoch (01-Jan-1970 00:00:00 UTC), `-1`     |
|                                          | if socket connection attempt did not occur                        |
+------------------------------------------+-------------------------------------------------------------------+
| `socketConnect.durationNanos`            | the duration took to connect to a remote peer, `-1` if socket     |
|                                          | connection attempt did not occur                                  |
+------------------------------------------+-------------------------------------------------------------------+
| `pendingAcquisition.startTimeMicros`     | when the client started to wait for the completion of an existing |
|                                          | connection attempt, in microseconds since the                     |
|                                          | epoch (01-Jan-1970 00:00:00 UTC), `-1` if waiting did not occur   |
+------------------------------------------+-------------------------------------------------------------------+
| `pendingAcquisition.durationNanos`       | the duration took to wait for the completion of an existing       |
|                                          | connection attempt to use one connection in HTTP/2, `-1` if       |
|                                          | waiting did not occur                                             |
+------------------------------------------+-------------------------------------------------------------------+

The connection acquisition duration is the sum of `dnsResolution.durationNanos`, `socketConnect.durationNanos` and
`pendingAcquisition.ㅇurationNanos`. They may or may not occur depending on circumstances.
These are some of the scenarios how the total duration is composed of:

1. A request resolves a domain name, and then checks if there is an existing connection
that can be used. Afterwards, a socket connection is made to the remote peer.
...
2. First a request checks if there are any available existing connections.
Afterwards, the request waits for another in-flight request to establish a connection.
(Note that, if you create a client with an IP address, `dnsResolution` did not
...
3. Connecting to the remote peer with the resolved IP address after the pending connection attempt is
failed.
