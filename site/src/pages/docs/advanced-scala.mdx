...
* or just by using Armeria directly in your Scala projects.
In that case, the supplied API would be fully in `Java`.

If you're searching for the most Scala-way experience of using Armeria,
take a look at `armeria-scala` and `http4s-armeria`. These two libraries differentiate
