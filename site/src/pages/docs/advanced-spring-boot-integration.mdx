...
The Spring framework provides powerful features necessary for building web applications, such as dependency injection, data binding, AOP, and transaction management.
Armeria is a lightweight microservices framework that offers capabilities comparable to existing full-stack web frameworks.
It has Rich support for [Apache Thrift](https://thrift.apache.org/) and [gRPC](https://grpc.io/), including a user-friendly [web console](https://armeria.dev/docs/server-docservice)
that allows you to send Thrift and gRPC requests directly from a web browser. Armeria supports both HTTP/1 and HTTP/2,
and these protocols both support [cleartext](https://simple.wikipedia.org/wiki/Cleartext) and TLS cryptographic protocols.
Armeria utilizes an [event loop](https://en.wikipedia.org/wiki/Event_loop) based on [Netty](https://netty.io/index.html),
enabling non-blocking I/O operations. As a result, it demonstrates better performance in certain scenarios, such as media streaming or
large file transfers. By integrating your Spring application with Armeria, you can serve both legacy Spring services
and any services built with Armeria, such as gRPC or Thrift, on the same port. Additionally, you can gradually migrate your services,
...
<Tip>
  ...
  Visit [armeria-examples/spring-boot-tomcat](https://github.com/line/armeria/tree/main/examples/spring-boot-tomcatj) to
...
Serve the embedded Tomcat service via Armeria by using the <type://ArmeriaServerConfigurator> bean.
...
Although it may not seem like much has changed compared to before, your application is now ready to incorporate RPC services
(such as gRPC or Thrift) or Armeria's annotated service for RESTful service.
...
<Tip>
  ...
  Visit [armeria-examples/spring-boot-minimal](https://github.com/line/armeria/tree/main/examples/spring-boot-minimal) to
  find a fully working example.
...
Armeria's annotated service is similar to a Spring controller. Spring will automatically inject the required dependencies into the annotated service.
You can declare the annotated service as a bean and inject it into the server building process.
Here's an example:
...
// Make this annotated service as bean.
@Controller
@PathPrefix("/todos")
...
The service IDs are docs, health, metrics, and actuator. You can include one or multiple service IDs based on your requirements.

Armeria supports four internal services that are useful for monitoring and management purposes.
You can enable these services by including their service IDs in your `application.yml` (or `application.properties`) file.
Service ids are `docs`, `health`, `metrics`, `actuator`. You can include one or multiple service IDs based on your requirements.
...
For instance, it allows you to test RPC protocols in a web browser console, similar to Swagger.
...
For example, the below code is checking tomcat connector state.
...
You can expose the collected metrics to various monitoring systems, such as [Prometheus](https://prometheus.io) or [Dropwizard](https://metrics.dropwizard.io/), for comprehensive monitoring and analysis.
...
<Tip>
  ...
  to avoid potential expected problems.
...
Please note that you should avoid configuring the aforementioned settings directly using <type://ServerBuilder> through
<type://ArmeriaServerConfigurator> or a `Consumer<ServerBuilder>` bean. These configurations will be overridden by the automatic configuration of Armeria.

### Actuator support

If you are using [Spring Boot Actuator](https://docs.spring.io/spring-boot/docs/current/actuator-api/htmlsingle/),
you can serve it at the `armeria.internal-services.port`, and the internal services can be served at the `management.server.port`.
...
Armeria provides the ability to manually inject dependencies using <type://DependencyInjector>. You can refer to the example in the [1.17.0 release notes](/release-notes/1.17.0) for more details.

Create the <type://DependencyInjector> bean, which will replace the default dependency injector. It's important to note that in this case,
you should be aware that instances that were automatically injected before may not be injected anymore.
