...
- Implementing [DecoratingHttpClientFunction](type) and [DecoratingRpcClientFunction](type)
- Extending [SimpleDecoratingClient](type)

## Implementing `DecoratingHttpClientFunction` and `DecoratingRpcClientFunction`

[DecoratingHttpClientFunction](type) and [DecoratingRpcClientFunction](type) are functional interfaces that
...
[SimpleDecoratingHttpClient](type) or [SimpleDecoratingRpcClient](type) depending on whether
you are decorating an [HttpClient](type) or an [RpcClient](type):
...
If the decorator modifies the response (e.g. [DecodingClient](type)) or spawns more requests
(e.g. [RetryingClient](type)), the outcome may be different depending on the order of the decorators.
Let's look at the following example that [DecodingClient](type) and [ContentPreviewingClient](type)
are used together:
...
[DecodingClient](type) decodes the content of HTTP responses. [ContentPreviewingClient](type) is
[Enabling content previews](/docs/advanced/structured-logging#enabling-content-previews)
of the HTTP response by setting it to the [RequestLog](type).
Because it's evaluated after [DecodingClient](type) from the point of view of an
...
preview because [ContentPreviewingClient](type) is evaluated first for the HTTP response.
For [RetryingClient](type), please check out
[`RetryingClient` with logging](/docs/client/retry#retryingclient-with-logging).

## Modifying responses

We can modify the responses in a decorator via the [HttpResponse#mapHeaders(Function)](type),
[HttpResponse#mapData(Function)](type), [HttpResponse#mapInformational(Function)](type)
or [HttpResponse.mapTrailers(Function)](type) methods, or, if more advanced/stateful
processing is necessary, by wrapping the [HttpResponse](type) in a [FilteredHttpResponse](type).
...
### Using [FilteredHttpResponse](type):

A couple of caveats to note here.

- Informational response headers may occur zero or more times.
- [HttpData](type) will almost always just be a part of the whole response data
  unless the response has been passed through [HttpResponse#aggregate()](type).
...
look at the [HttpData](type), it can be achieved in a more performant way via
`ClientRequestContext.log().whenAvailable(RequestLogProperty).thenAccept(...)`. Accessing the
[ClientRequestContext](type) can be done via either wrapping in a decorator (and using the ctx or by using
[ClientRequestContextCaptor](type):
...
- [`RequestContext` custom attributes](/docs/advanced/logging/#custom-attributes)
