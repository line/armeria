...
- You can now set an <type://ExchangeType> to <type://HttpService> for whether to enable streaming. #3956
  - Non-streaming services will show better performance when sending and receiving a small size of data.
  - However, if streaming is disabled, a request and response will be buffered before it is sent or received.
    So it is recommended to enable streaming when handling a large size of data.
  ```java
  class MyRestService implements HttpService {
    @Override 
    ExchangeType exchangeType(RequestHeaders headers, Route route) {
...
- You can now fluently build an <type://HttpResponse> by using <type://HttpRequestBuilder>. #3398 #3941
  ...
  HttpResponse.builder()
              ...
              .build(); 
  ```
- You can now use <type://StreamMessage.mapAsync(CompletableFuture)> to transform a <type://StreamMessage> 
...
- You can now use `{*variable}` and `:variable` path patterns to capture rest paths of a request. #3031 #3997
  ```java
  Server.builder()
...
- You can now force a state transition of a <type://CircuitBreaker>
  ...
  circuitBreaker.enterState(CircuitState.OPEN);
  ```
...
- You can now dynamically change the maximum number of concurrent active requests of
  ...
  ```
- We can get the content and trailers of an <type://HttpRequest> separately by using
...
- You can now use [OpenMetrics](https://github.com/OpenObservability/OpenMetrics/blob/main/specification/OpenMetrics.md)
  format with <type://PrometheusExpositionService>. #3926 #3928
...
- You can now can see the package name of their services, messages and enums with the same names from 
  <type://DocService>. #3979
- Used `ByteBuf` to avoid creating a large `byte[]` in <type://PrometheusExpositionService>. #3929
...
- You no longer see `ClassCastException` when duplicating an `HttpRequest` or an `HttpResponse`. #4030 #4032
...
- Improved <type://Multipart> decoder to correctly publish chunks one by one when streaming 
