...
In this step, write the code required for service itself. To retrieve a single blog post information, copy the following code inside the `getBlogPost()` method:
...
<TabPane tab="Read multiple posts" key="2">
...
In this step, write the code required for service itself. To retrieve multiple blog posts, copy the following code inside the `getBlogPosts()` method. Note that the return type has been changed from `void` to `Iterable<BlogPost>`.
