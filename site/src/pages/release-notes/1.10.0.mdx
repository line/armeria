...
- A new module `armeria-sangria` integrates with [Sangria GraphQL](https://sangria-graphql.github.io) to let you
  serve a GraphQL request in Scala. #3703 #3704
  ```scala
...
- You can now recover a failed <type://HttpResponse> with a fallback <type://HttpResponse>. It would be useful
  for handing an error of <type://HttpResponse> in a decorator. #3674
  ```java
...
- You can now resume a failed <type://StreamMessage> with a fallback <type://StreamMessage>. #3674
  ...
  ```
- You can now transform an error of <type://StreamMessage> into another using a function. #3668
  ```java
...
- You can now compress and decompress content with [Brotli](https://www.brotli.org/) when using
  <type://EncodingService>, <type://DecodingService> and <type://DecodingClient>.
- You can now create an <type://HttpResponseException> with a cause. #3674
- You can now easily capture <typeplural://ServiceRequestContext> using <type://ServerExtension> in tests.
- You can now suppress the Netty version warning by specifying
  `-Dcom.linecorp.armeria.warnNettyVersions=false`. #3572 #3766

## 📃 Documentations

- You can now learn how to write a service with Armeria by walking through the
...
- <type://ConcurrencyLimitTimeoutException> is now raised when a request times out
  in <type://ConcurrencyLimitingClient>.
