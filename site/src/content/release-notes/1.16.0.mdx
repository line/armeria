...
- Armeria [Server](type) now exposes the metrics about TLS handshake results. #4191
  ```
  ...
  ```
- You can use the [@Delimiter](type) annotation in an annotated service to specify the delimiter of
  ...
  - You can also specify a delimiter programmatically with [AnnotatedServiceBindingBuilder#queryDelimiter(String)](type)
- You can now use the no-op `subscribe()` to drain elements from a [StreamMessage](type) #4145 #4185
  ```java
...
- You can now use the `response_body` option in gRPC-JSON transcoding. #4052 #4132 #4174
  - This option is useful when you need to make a certain response field as a response body.
    ...
    ```
- You can now use the [@Decorator](type) annotation to apply decorators to your gRPC stubs. #3967 #4041
  ```java
  @LoggingDecorator // 👈👈👈
  ...
  ```
- [AbstractUnaryGrpcService](type) and unframed [GrpcService](type) now include the stack trace for failed
  requests when `verboseResponses` option is enabled for easier debuggability. #4114 #4171
- You can now specify query parameters additively by calling [WebClientRequestPreparation#queryParam(String,Object)](type)
  #4173
  ```java
  ...
  WebClient
    ...
    .execute()
  ```
- You can now override the global [Flags](type) by implementing your own [FlagsProvider](type) and
...
- When using `armeria-graphql`, you can now retrieve the current [ServiceRequestContext](type) with
  [GraphqlServiceContexts#get(DataFetchingEnvironment)](type) #4201
  ```java
  new DataFetcher<>() {
    @Override
  ...
  ```
- You can now disable the warning message about masked routes when building a [Server](type) by specifying
...
- JSON-gRPC transcoding now works even if you bind a [GrpcService](type) with path prefix. #4086 #4215
  ```java
  Server
    .builder()
    .route()
...
- [EurekaEndpointGroup](type) now continues refreshing itself even if it failed to refresh once. #4206
- [HealthCheckedEndpointGroup](type) now leaves a detailed error message if all endpoints are unhealthy
  on the first check. #4200
- Fixed a rare infinite loop in [EndpointSelectionStrategy#rampingUp()](type) #4219
...
- Reduced the cardinality of TLS certificate expiry metrics by using a hostname pattern instead of a hostname
  as a label. #4191
- [TomcatService](type) now records the request start time of `CoyoteRequest` correctly. #4182 #4183
...
- You don't get the noisy warning message about double-completion of future, which occurs when a user
  misconfigured a [ClientFactory](type) #4191
...
- [Flags#verboseExceptionSamplerSpec()](type) has been deprecated in favor of
  [Flags#verboseExceptionSampler()](type) #4093 #4151
...
- The return type of [Flags#requestContextStorageProvider()](type) was changed from `String` to
  [RequestContextStorageProvider](type), so that a user knows which provider will be used exactly. #4093 #4151
