...
You can integrate your Spring MVC with Armeria using [TomcatService](type).
...
Avoid directly using the [ServerBuilder](type) method with [ArmeriaServerConfigurator](type) or `Consumer<ServerBuilder>` beans,
...
To customize the health check operation, you can utilize the [HealthChecker](type) bean.
...
If you intend to use a different monitoring system, you will need to utilize the [ArmeriaServerConfigurator](type) or
...
### [ArmeriaServerConfigurator](type) vs `Consumer<ServerBuilder>`

You can use both [ArmeriaServerConfigurator](type) and `Consumer<ServerBuilder>` to configure the server using [ServerBuilder](type).
...
Armeria first configures all [ArmeriaServerConfigurator](type) beans and subsequently applies all `Consumer<ServerBuilder>` beans.

If you have multiple [ArmeriaServerConfigurator](type) or `Consumer<ServerBuilder>` beans, you can set the order
...
It is important to note that the default order of [ArmeriaServerConfigurator](type) is zero,
...
Armeria provides the ability to manually inject dependencies using [DependencyInjector](type).
