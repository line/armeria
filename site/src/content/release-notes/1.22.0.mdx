...
- [CircuitBreakerClient](type) integration with Resilience4j's circuit breaker is now available. #1469 #4139
  ...
  WebClient client = WebClient
    ...
    ...
  ```
- You can now use [AsyncServerInterceptor](type) to asynchronously intercept gRPC service calls. #4647
...
- [@Blocking](type) annotation can now be used with gRPC services. #4389 #4585
  ```java
...
- The authenticated remote user set by [RequestLogBuilder#authenticatedUser(String)](type) is now logged in access
  logs. #4634
- [DocService](type) now exposes [JSON Schema](https://json-schema.org/) via the `/schemas.json` endpoint. #4518

{/* truncate */}

## 📈 Improvements

- [Server#reconfigure(ServerConfigurator)](type) now dynamically updates TLS configurations correctly. #4637
- Thrift [typedefs](http://diwakergupta.github.io/thrift-missing-guide/#_typedefs) are
  now correctly displayed in [DocService](type). #4628
- [DocService](type) front-end assets are now compressed using gzip instead of Brotli. #4616
- An exception is raised if TLS is set for a port-based virtual host. #4588

## 🛠️ Bug fixes

- A `NullPointerException` is not raised anymore when an aborted [StreamMessage](type) is collected. #4652
- A [ClosedSessionException](type) is not set to [RequestLog#responseCause()](type) anymore when
...
- `431 Request Header Fields Too Large` is now returned instead of `400 Bad Request` if the header size exceeds [ServerConfig#http1MaxHeaderSize()](type). #4609 #4655
- Warnings on duplicate parameters in [DocService](type) are no longer visible when annotations are added to Kotlin data classes. #4645
...
- gRPC metadata is now exposed via [StatusRuntimeException](type) in [RequestLog#responseCause()](type). #4635 #4636
- Square brackets "`[]`" that are specified in path segments are correctly encoded. #4607 #4615
- [DecodingService](type) and [DecodingClient](type) now correctly limit the max length of decompressed streaming data. #4613
- Client-side gRPC callback `ClientCall.Listener.onHeaders()` is now invoked correctly. #4583 #4608
- [DocService](type) now correctly displays multiple example requests for a single endpoint. #4576 #4605
