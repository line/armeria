...
We've only tested running the service on a server. This time, let's try calling the service. To test, we'll write a simple client using Armeria's <type://Clients.newClient> and the Java code generated from the [Defining a service](/tutorials/grpc/blog/define-service).
...
While the server is running, run the `main()` method on the client. You'll get an exception.
