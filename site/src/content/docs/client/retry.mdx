...
- [RetryingClient](type)
- [RetryingRpcClient](type)

Both behave the same except for the different request and response types.
So, let's find out what we can do with [RetryingClient](type).

## `RetryingClient`

You can just use the `decorator()` method in [ClientBuilder](type) or [WebClientBuilder](type) to build a
[RetryingClient](type). For example:
...
[RetryingClient#newDecorator(RetryRule,int)](type). Meanwhile, the `rule` will decide to
retry depending on the response. In this case, the client retries when it receives `5xx` response error or
an exception is raised.

## `RetryRule`

You can fluently build your own [RetryRule](type).
...
Or you can customize the **rule** by implementing [RetryRule](type).
...
This will retry when one of [ResponseTimeoutException](type) and [UnprocessedRequestException](type) is raised.
...
We declare a [Backoff](type) as a member and reuse it when a `rule` returns it, so that we do not
return a different [Backoff](type) instance for each `shouldRetry()`. [RetryingClient](type)
internally tracks the reference of the returned [Backoff](type) and increases the counter that keeps
the number of attempts made so far, and resets it to 0 when the [Backoff](type) returned by the retry rule
is not the same as before. Therefore, it is important to return the same [Backoff](type) instance unless
you decided to change your [Backoff](type) strategy. If you do not return the same one, when the
[Backoff](type) yields a different delay based on the number of retries, such as an exponential backoff,
it will not work as expected. We will take a close look into a [Backoff](type) at the next section.

:::

:::tip

[UnprocessedRequestException](type) literally means that the request has not been processed by the server.
...
You can return a different [Backoff](type) according to the response status.
...
[RetryRuleWithContent](type) and specify it when you create a [WebClient](type)
using [RetryingClientBuilder](type):
...
You might find the [Exceptions#peel(Throwable)](type) method useful when the exception you are trying to
...
You can use a [Backoff](type) to determine the delay between attempts. Armeria provides [Backoff](type)
implementations which produce the following delays out of the box:

- Fixed delay, created with [Backoff#fixed(long)](type)
- Random delay, created with [Backoff#random(long,long)](type)
- Exponential delay which is multiplied on each attempt, created with [Backoff#exponential(long,long)](type)

Armeria provides [Backoff#ofDefault()](type) that you might use by default. It is exactly the same as:
...
retry. Please note that the [Backoff#withJitter(double)](type) will add jitter value to the calculated delay.
...
If you create a [Backoff](type) using [Backoff#withMaxAttempts(int)](type) in a [RetryRule](type),
the [RetryingClient](type) which uses the [RetryRule](type) will stop retrying when the number of
attempts passed `maxAttempts`. However, if you have more than one [Backoff](type) and return one after
the other continuously, it will keep retrying over and over again because the counter that
[RetryingClient](type) internally tracks is initialized every time the different [Backoff](type) is
returned. To limit the number of attempts in a whole retry session, [RetryingClient](type) limits
the maximum number of total attempts to 10 by default. You can change this value by specifying
`maxTotalAttempts` when you build a [RetryingClient](type):
...
Note that when a [RetryingClient](type) stops due to the attempts limit, the client will get the last received
[Response](type) from the server.

## Per-attempt timeout

[ResponseTimeoutException](type) can occur in two different situations while retrying. First, it occurs
...
You cannot retry on this [ResponseTimeoutException](type).
...
You can retry on this [ResponseTimeoutException](type).
...
[Backoff](type), the first three attempts will be timed out by the per-attempt timeout (3,000ms).
...
In the example above, every attempt is made before it is timed out because the [Backoff](type) is disabled.
However, what if a [Backoff](type) is enabled and the moment of trying next attempt is after the point of
[ResponseTimeoutException](type)? In such a case, the [RetryingClient](type) does not schedule for the
next attempt, but finishes the retry session immediately with the last received [Response](type).
...
Unlike the example above, the [Backoff](type) is enabled and it makes the [RetryingClient](type) perform
...
The [RetryingClient](type), at this point, stops retrying and finished the retry session with the last
received [Response](type), retrieved at 9,000ms from the attempt 2.

## `RetryingClient` with logging

You can use [RetryingClient](type) with [LoggingClient](type) to log. If you want to log all of the
requests and responses, decorate [LoggingClient](type) with [RetryingClient](type). That is:
...
[RetryingClient](type) inserts it to indicate the retry count of a request.
...
You might want to use [Circuit breaker](/docs/client/circuit-breaker) with [RetryingClient](type) using
...
This decorates [CircuitBreakerClient](type) with [RetryingClient](type) so that the [CircuitBreaker](type)
judges every request and retried request as successful or failed. If the failure rate exceeds a certain
threshold, it raises a [FailFastException](type). When using both clients, you need to build a custom
[RetryRule](type) to handle this exception so that the [RetryingClient](type) does not attempt
...
You may want to allow retrying even on [FailFastException](type) when your endpoint is configured with
