...
final class ScalaPbRequestConverterFunction private (jsonParser: Parser, resultType: ResultType.Value)
  ...
  override def convertRequest(
    ...
    if (resultType == PROTOBUF ||
      (resultType == UNKNOWN && isProtobufMessage(expectedResultType))) {
...
object ScalaPbRequestConverterFunction {
  ...
  private val toOneofMethodCache: ConcurrentMap[Class[_], Method] =
    ...
    val defaultInstance = defaultInstanceCache.computeIfAbsent(
      ...
      key =>
        try {
          ...
          val method = genClass.getDeclaredMethod("defaultInstance")
          method.invoke(null).asInstanceOf[GeneratedMessage]
        } catch {
  ...
  private def toGenerateMessageOrOneof(
    ...
    if (classOf[GeneratedSealedOneof].isAssignableFrom(expectedResultType)) {
      val method = toOneofMethodCache.computeIfAbsent(
        expectedResultType,
        key =>
          try {
            result.getClass.getDeclaredMethod(s"to${key.getSimpleName}")
          } catch {
      ...
      if (method eq unknownMethod)
        throw new IllegalStateException(
          s"Failed to find to${expectedResultType.getSimpleName} method from $result")

      method.invoke(result).asInstanceOf[Any with Serializable]
    } else
      result
  ...
  private[scalapb] val unknownMethod: Method = {
    val method = classOf[String].getDeclaredMethod("valueOf", classOf[Any])
    assert(method != null)
    method
  }
}
