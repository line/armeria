...
And here, we'll learn how to handle an exception with a custom exception handler.
...
1. Bind the exception handler to the service for the test server.
  ...
  ```
2. Add a test method to update a blog post with an invalid ID, asserting an `BlogNotFoundException` exception is thrown.
  ```java filename=BlogServiceTest.java
  @Test
  @Order(6)
