...
## 1. Update the thrift file for server

Let's add a couple of structs and a service method to the `blog.thrift` file.
Here, we will add only the creation method and a struct that for creating blog posts.
We will add the retrieving, modification, and deletion method at each step.

1. Add structs, `BlogPost` and `CreateBlogPostReaust`.
  ...
  ```
2. In `BlogService`, remove the `hello()` method and add a method, `createBlogPost()`.
  ```thrift filename=blog.thrift
  service BlogService {
      /**
       * Creates a blog post.
  ...
  ```
3. To generate Java code, compile the `blog.thrift` file like we did in [Step 1. Create a service](](/tutorials/thrift/blog/create-service#2-compile-the-thrift-file)).

## 2. Implement server-side

Let's implement a service method `createBlogPost()` in `BlogServiceImpl.java`.

1. Remove the `hello()` method.
2. Override a service method, `createBlogPost()`.
  ...
  ```
2. Create an ID generator to issue temporary blog post IDs and a map to contain blog posts.
  ```java filename=BlogServiceImpl.java
  ...
  ```
3. Implement the `createBlogPost()` method:
    1. Create a `BlogPost` object with request parameters.
    2. Save the post information in the `blogPosts` map.
    3. Return the information of the blog post created to the `resultHandler`.
  ```java filename=BlogServiceImpl.java
...
## 3. Update the thrift file for client

Update the `blog.thrift` file for client, like we did in [1. Update the thrift file for server](#1-update-the-thrift-file-for-server).

## 4. Implement client-side

Add a method `createBlogPost()` in `BlogClient.java` to send a request to create a blog post.

1. In the `main()` method, remove two lines of code calling the `hello()` method.
  ...
  ```
2. Add a method `createBlogPost()` to send a request to create a blog post.
  ```java filename=BlogClient.java
  import example.armeria.blog.thrift.BlogPost;
  import example.armeria.blog.thrift.CreateBlogPostRequest;
...
Now, let's make a request from the client. Note the code is minimal to keep things simple.
