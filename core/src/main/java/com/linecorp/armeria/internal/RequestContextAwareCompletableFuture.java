...
final class RequestContextAwareCompletableFuture<T> extends CompletableFuture<T> {
    ...
    public CompletableFuture<Void> runAfterBothAsync(CompletionStage<?> other,
                                                     Runnable action,
                                                     Executor executor) {
        return ctx.makeContextAware(super.runAfterBothAsync(
    ...
    public CompletableFuture<Void> acceptEitherAsync(CompletionStage<? extends T> other,
                                                     Consumer<? super T> action,
                                                     Executor executor) {
        return ctx.makeContextAware(super.acceptEitherAsync(
                other, ctx.makeContextAware(action), executor));
    }
    ...
    public CompletableFuture<Void> runAfterEitherAsync(CompletionStage<?> other,
                                                       Runnable action,
                                                       Executor executor) {
        return ctx.makeContextAware(super.runAfterEitherAsync(
                other, ctx.makeContextAware(action), executor));
    }
