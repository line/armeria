...
class AthenzAccessTokenRpcFilterTest {
    ...
    static final ServerExtension echoServer = new ServerExtension() {
        ...
        protected void configure(ServerBuilder sb) {
            sb.service("/", THttpService.of((EchoService.Iface) () -> {
                final String auth = ServiceRequestContext.current().request()
                                                         .headers().get("authorization");
                return auth != null ? auth : "no-auth";
            }));
            sb.http(0);
        }
    };
    ...
    @Test
    void tokenInjectedViaRpcPreprocessor() throws Exception {
        ...
        try (XdsBootstrap xdsBootstrap = XdsBootstrap.of(bootstrap, eventLoop.get());
             XdsRpcPreprocessor preprocessor =
                     XdsRpcPreprocessor.ofListener(LISTENER_NAME, xdsBootstrap)) {
            final EchoService.Iface client =
                    ThriftClients.newClient(preprocessor, EchoService.Iface.class);
            final String result = client.echoAuth();
