...
authors:
  - ikhunum
other_languages:
  ko: /blog/ko/2020/02/19/reactive-streams-armeria-2
  ja: /blog/ja/2020/03/26/reactive-streams-armeria-2
---

# Let's Play with Reactive Streams on Armeria - Part 2

In the [first part of this blog post](/blog/2020/05/06/reactive-streams-armeria-1), we took a look at the basic concepts of Reactive Streams.
...
For example, you can easily write routing code by setting `hello` as a prefix, and `name` as a path variable like the code below (For more information on using annotations, refer to [official Armeria documentation](/docs/server-annotated-service)).
...
You can use a separate [decorator](/docs/server-decorator) to manage this [separation of concerns](https://en.wikipedia.org/wiki/Separation_of_concerns).[[2]](#footnotes)
...
If you are already using [Spring WebFlux](https://docs.spring.io/spring/docs/current/spring-framework-reference/web-reactive.html) to use Reactive Streams, you only need to [add](/docs/advanced-spring-webflux-integration)`armeria-spring-boot-webflux-starter`[to your dependencies](/docs/advanced-spring-webflux-integration) to complete migration to Armeria.
...
- Kubernetes-type DNS (Domain Name System) and [service discovery with ZooKeeper](/docs/server/service-registration#zookeeper-based-service-registration-with-zookeeperupdatinglistener) for easily locating servers in a cloud environment.
...
For more information on what Armeria can do for you, refer to the [official website](/) and the following sources.

- GitHub: [line/armeria](https://github.com/line/armeria)
- Twitter: [@armeria_project](https://twitter.com/armeria_project)
- Chat: [Discord](/s/discord)
- [API documentation](https://javadoc.io/doc/com.linecorp.armeria/armeria-javadoc/latest/index.html), [release notes](/release-notes/)
...
1. Refer to [Running a gRPC service](/docs/server-grpc) and [Running a Thrift service](/docs/server-thrift).
