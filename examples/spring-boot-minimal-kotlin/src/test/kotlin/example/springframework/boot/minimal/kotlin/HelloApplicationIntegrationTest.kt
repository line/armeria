...
class HelloApplicationIntegrationTest (@Autowired server: Server) {
    ...
    fun success() {
        ...
        Assertions.assertThat(response.contentUtf8())
    ...
    fun failure() {
        ...
        Assertions.assertThat(response.status()).isEqualTo(HttpStatus.BAD_REQUEST)
        JsonFluentAssert.assertThatJson(response.contentUtf8())
                        .node("message")
