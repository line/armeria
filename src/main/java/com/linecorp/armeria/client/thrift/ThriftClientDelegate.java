...
final class ThriftClientDelegate implements Client {
    ...
    public ThriftReply execute(ClientRequestContext ctx, Request req) throws Exception {
        ...
        ctx.requestLogBuilder().attach(call);
        ctx.responseLogBuilder().attach(reply);

        final ThriftFunction func;
        try {
            func = metadata(call.serviceType()).function(method);
            if (func == null) {
                throw new IllegalStateException("Thrift method not found: " + method);
            }
    ...
    private ThriftServiceMetadata metadata(Class<?> clientType) {
        final ThriftServiceMetadata metadata = metadataMap.get(clientType);
        if (metadata != null) {
            return metadata;
        }

        return metadataMap.computeIfAbsent(clientType, ThriftServiceMetadata::new);
    }
