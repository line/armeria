...
- `blog.thrift` for each server and client
...
Let's update the server's `blog.thrift` file we created earlier in [Step 1. Create a service](/tutorials/thrift/blog/create-service).
...
Let's write and compile the thrift file for client as well, then update the client code accordingly.
