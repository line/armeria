...
1. **User decorators** — any decorators added via `WebClient.builder(preprocessor).decorator(...)` by the user.
2. **Downstream HTTP filters** — xDS filters declared in the Listener's `http_filters`.
3. **Router** — matches the request to a route, applies retry policy, selects an endpoint
   using the cluster's load balancing policy, and determines the protocol and TLS parameters.
4. **Upstream HTTP filters** — per-route filters declared on the router.
