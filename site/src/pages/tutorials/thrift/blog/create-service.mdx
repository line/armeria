...
In the thrift file, let's define the `BlogService` service with a method `hello()`.
...
See [Sample service](/tutorials/thrift/blog#sample-service) for the overall folder structure of the sample service.
...
You can refer to the full [build.gradle](https://github.com/line/armeria-examples/blob/master/tutorials/thrift/build.gradle) file for generating code with [Gradle Thrift Plugin](https://github.com/jruyi/thrift-gradle-plugin).
...
2. Declare the `BlogServiceImpl` class and override the `hello()` method. The `BlogServiceImpl` class implements the `BlogService` service we defined in the earlier step.
  ...
  ```
3. Add a dummy result for the service method.
  This is for temporary use to test the connection between server and client.
  ```java filename=BlogServiceImpl.java
  @Override
  public void hello(AsyncMethodCallback<String> resultHandler) {
      resultHandler.onComplete("Hello, Armeria!"); // Add this
  }
  ```
...
Next, at [Step 2. Run a server and client](/tutorials/thrift/blog/run-server-and-client), we'll create and run a server with the service we've created.
