...
Previously, we created and read blog posts. Now, let's implement and make a call to update a blog post. Here, we'll throw a custom exception and add a <type://GrpcExceptionHandler> for handling.
...
1. In the BlogService.java file we've been working on, add a method for 
updating a blog post, `updateBlogPost()`. Leave the exception throwing part empty for now.
...
3. Add an exception handler class for the blog service.
  ```java GrpcExceptionHandler.java
  ...
  ```
4. Bind the exception handler, <type://GrpcExceptionHandler>, to our service.
...
We'll continue using the `cb` instance that has the logging interceptor, as the client. Add a method `updateBlogPost()` to send a request to update a blog post.
...
1. Add a method to test run the update method we just implemented. 
  ...
  ```
2. [Re-run the server](/tutorials/grpc/blog/run-service#3-serve-the-service), since we've modified the server-side in [1. Implement service](#1-implement-service).
...
To check our exception handler working, let's try updating a post we know for sure does not exist.
...
1. Add a method to test run the update method we just implemented.
  ...
  ```
2. While the server is running, run the client and see the message that you get. We threw the `BlogNotFoundException` and the exception handler returned the not found runtime exception for us.
```bash
Exception in thread "main" io.grpc.StatusRuntimeException: NOT_FOUND: The blog post does not exist. ID: 10000
	...
	at example.armeria.server.blog.grpc.BlogServiceGrpc$BlogServiceBlockingStub.updateBlogPost(BlogServiceGrpc.java:390)
...
blog post. Plus, we added exception handling. Next, we'll implement a method for [deleting a blog post](/tutorials/grpc/blog/implement-delete).
