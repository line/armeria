...
We can modify the responses in a decorator via the `HttpResponse.mapHeaders(...)`, `HttpResponse.mapData(...)`,
`HttpResponse.mapInformational(...)` or `HttpResponse.mapTrailers(...)` methods, or, if more advanced/stateful
processing is necessary, by wrapping the <type://HttpResponse> in a <type://FilteredHttpResponse>.

### Using `.mapXXX(...)` methods
...
* <type://HttpData> will almost always just be a part of the whole response data unless the response has been passed through `HttpResponse.aggregate()`.
