...
How are we going to serve that file?
...
```flowchart
st=>start: Divide the file into chunks
e=>end: We sent all chunks
op1=>operation: Set i = 0
op2=>operation: Load the ith chunk
cond=>condition: Is the chunk empty?

op3=>operation: Send the chunk to the client
op4=>operation: Load the ith chunk into memory
op5=>operation: Send the chunk to the client
op6=>operation: Increase i by 1
op7=>operation: We sent all chunks

st->op1->op2->cond
cond(yes)->e
cond(no, right)->op5(right)->op6(right)->op2
```
...
## Sending a file using <type://HttpFile>

It's always easy to say, but what should we do to send the file with backpressure using Armeria?
```java
...
Server.builder()
      .service("/bif_file.dat", bigFile.asService())
      .build();
```
That's it! The <type://HttpFile> loads chunks one by one and send them to the client with backpressure.

## Sending a streaming response using <type://HttpResponseWriter>

What if you want to send large data from other than files (e.g. database) with backpressure?
To answer that question, let's implement the simplified version of <type://HttpFile>.

First, we use <type://HttpResponseWriter> from <type://HttpResponse#streaming()> to send streaming responses:
```java
...
```
So far we have implemented the simplified version of <type://HttpFile>. Now, we can implement streaming
responses with backpressure from any other sources (e.g. database) by simply changing the `produceChunk()`
...
[RxJava](https://github.com/ReactiveX/RxJava). Then, you can simply return it using
