...
- **Model Context Protocol (MCP) Server**: You can now build [MCP](https://modelcontextprotocol.io/) services on top of Armeria using the `armeria-ai-mcp` module. #6513 #6492
  ...
  Server server =
    Server.builder()
          ...
          .build();
  ```
- **Retry Limiting**: You can now limit retry requests using [RetryLimiter](type) to prevent system overload during prolonged service degradation. #6409
  ```java
  RetryRule rule = ...
  RetryConfig config =
...
- **Support for Multipart Requests with Duplicate Names**: You can now handle multipart requests that contain multiple parts with the same form field name. #6419 #6430
  ...
  ```
- **AnnotatedService with JSpecify**: You can now use [AnnotatedService](type) with [JSpecify](https://jspecify.dev/)'s `@Nullable` annotations. #6454 #6457
  ```java
  ...
  ```
- **HTTP Compression Algorithm Control**: You can now configure which compression algorithms to use with [EncodingService](type). #6390 #6398
  ```java
...
- **SAML RelayState Length Configuration**: You can now configure the maximum length for SAML RelayState parameters. #6381
  ...
  SamlServiceProvider sp =
    SamlServiceProvider
        ...
        ...
  ```
- **Easier Certificate Chain Testing**: You can now use [SignedCertificateExtension](type) to generate certificate chains. #6519
...
- - You may adjust stacktrace generation for [CancelledSubscriptionException](type) using `verboseExceptionSampler`. #6525

## 🛠️ Bug fixes

- [ServiceRequestContext](type) validation has been improved to avoid false-positive exceptions. #6420 #6479
  - Pushing a [ServiceRequestContext](type)-based [RequestContext](type) to a thread that does not already have a [ServiceRequestContext](type) no longer emits an [IllegalStateException](type).
- Resolved duplicate decorator registration in [ZtsBaseClient](type) when using [ArmeriaAutoConfiguration](type). #6424
- [ContentPreviewerFactory](type) no longer unnecessarily logs content for binary types. #6485
...
- [RejectedRouterHandler#FAIL](type) correctly throws an exception when duplicate routes are detected. #6498 #6499
- JSON formatting has been fixed in the [DocService](type) debug console. #6434
- [DocService](type) no longer sends `Content-Type` headers for HTTP methods that don't have request bodies. #6442
