...
[RequestLog](type) provides various properties recorded while handling a request:

### Request properties

| <div style={{width: '13rem'}}>Property</div> | Description                                                                                                                                                                                                                                                                                              |
| -------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `requestStartTimeMicros`                     | when the request processing started, in microseconds since the epoch (01-Jan-1970 00:00:00 UTC)                                                                                                                                                                                                          |
...
| `name`                                       | the human-readable simple name of the request, such as: - gRPC - A capitalized method name defined in `io.grpc.MethodDescriptor` (e.g, `GetItems`) - Thrift and annotated service - a method name (e.g, `getItems`) - [HttpService](type) - an HTTP method name                                          |
| `serviceName`                                | the human-readable name of the service that served the request, such as: - gRPC - a service name (e.g, `com.foo.GrpcService`) - Thrift - a service type (e.g, `com.foo.ThriftService$AsyncIface` or `com.foo.ThriftService$Iface)` - [HttpService](type) and annotated service - an innermost class name |
| `host`                                       | the name of the virtual host that serves the request                                                                                                                                                                                                                                                     |
| `requestHeaders`                             | the HTTP headers of the request. the header contains the method (e.g. `GET`, `POST`), the path (e.g. `/thrift/foo`), the query (e.g. `foo=bar&bar=baz`), the content type, etc.                                                                                                                          |
...
| <div style={{width: '13rem'}}>Property</div> | Description                                                                                             |
| -------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| `responseStartTimeMicros`                    | when the response processing started, in microseconds since the epoch (01-Jan-1970 00:00:00 UTC)        |
...
| `responseHeaders`                            | the HTTP headers of the response. the header contains the statusCode (e.g. 404), the content type, etc. |
...
For the server-side, `totalDurationNanos` starts when the server receives the [RequestHeaders](type) and
...
the [ResponseHeaders](type) is sent before the service receives the [HttpRequest](type) fully:
...
- The client sends the last frame of an [HttpRequest](type).
- The client receives a [ResponseHeaders](type).
...
| -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `connectionAcquisitionStartTimeMicros`       | when the client started to acquire a connection, in microseconds since the epoch (01-Jan-1970 00:00:00 UTC)                                                                     |
| `connectionAcquisitionDurationNanos`         | the duration took to get a connection (i.e. the total duration)                                                                                                                 |
| `dnsResolutionStartTimeMicros`               | when the client started to resolve a domain name, in microseconds since the epoch (01-Jan-1970 00:00:00 UTC), `-1` if DNS lookup did not occur                                  |
| `dnsResolutionDurationNanos`                 | the duration took to resolve a domain name, `-1` if DNS lookup did not occur                                                                                                    |
| `socketConnectStartTimeMicros`               | when the client started to connect to a remote peer, in microseconds since the epoch (01-Jan-1970 00:00:00 UTC), `-1` if socket connection attempt did not occur                |
| `socketConnectDurationNanos`                 | the duration took to connect to a remote peer, `-1` if socket connection attempt did not occur                                                                                  |
| `pendingAcquisitionStartTimeMicros`          | when the client started to wait for the completion of an existing connection attempt, in microseconds since the epoch (01-Jan-1970 00:00:00 UTC), `-1` if waiting did not occur |
| `pendingAcquisitionDurationNanos`            | the duration took to wait for the completion of an existing connection attempt to use one connection in HTTP/2, `-1` if waiting did not occur                                   |
...
The collected properties must be accessed via [RequestLogAccess](type), which provides a safe access to the
...
request and response properties, you need to use [ClientConnectionTimings](type) as follows:
...
The reason why we used the static method is that the [ClientConnectionTimings](type) is stored using
the attribute. See [RequestContext custom attributes](/docs/advanced/logging/#custom-attributes) for more information.

:::

## Enabling content previews

Armeria provides the `requestContentPreview` and `responseContentPreview` properties in [RequestLog](type)
...
by default. You can enable it using [ContentPreviewingClient](type) and [ContentPreviewingService](type)
decorators.
...
You can also customize the previews by specifying your own [ContentPreviewerFactory](type) implementation.
...
a single [RequestLog](type). A [RequestLog](type) can contain more than one child [RequestLog](type)
...
If the added [RequestLog](type) is the first child, the request-side log of the [RequestLog](type) will
...
illustrates how a [RequestLog](type) with child logs looks like:
...
[RetryingClient](type) is a good example that leverages this feature.
