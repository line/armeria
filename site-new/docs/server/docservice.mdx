...
[DocService](type) is a single-page web application which provides the following useful features:
...
First, add [DocService](type) to the [ServerBuilder](type):
...
[DocService](type) will scan for all supported services automatically when the [Server](type) starts up.
...
![](/img/docservice_2.png)

As you may have noticed, the 'description' column is empty. [DocService](type) can even show the docstrings
...
![](/img/docservice_5.png)

</MaxWidth>

## Including and excluding service methods
You can include or exclude service methods using [DocServiceFilter](type) when building a [DocService](type)
with a [DocServiceBuilder](type):
...
By adding examples to [DocService](type), your users will be able to play with the services you wrote
...
### For annotated service
  ...
  Using [@Description](type) annotation:
  ...
  You can render the description as [Markdown](https://github.github.com/gfm/) or [Mermaid](https://mermaid-js.github.io/mermaid/#/) using [Markup](type).
