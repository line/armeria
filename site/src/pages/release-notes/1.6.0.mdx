...
- You can now send and handle the multipart request. #253 #3327
  ```java
  ...
  ```
- Armeria now provides various useful extension and conversions for Scala. #3395
...
- You can now set a response timeout and attributes using <type://WebClientRequestPreparation>. #3347 #3357
  ...
  client.prepare()
        ...
        .execute();
  ```
- You can now add additional error details to the response when an exception is raised in gRPC service.
  #3307 #3329
  ```java
  GrpcService.builder()
...
- You can now fluently build an <type://HttpRequest> with a `Publisher<HttpData>`. #3343
  ```java
  Publisher<HttpData> publisher = StreamMessage.of(...);
  HttpRequest.builder()
...
- You can now check if the current request is matched by any routes or not. #3365 #3378
  ...
  ```
- The <type://BraveService> does not trace requests to the <type://TransientService> such as
...
- You can now clean up resources by overriding <type://FilteredStreamMessage#onCancellation(Subscriber)>
  ...
  ```
- You can now easily set 'cookie' or 'set-cookie' headers. #3391
  ```java
...
- You can now use `req.root_id` <type://BuiltInProperty> to log the id of <type://RequestContext#root()>.
  #3429 #3433
- You can now use Thrift 0.14.0. #3470 #3422
- You can now use OAuth 2.0 related features. #2268 #2840
...
- You no longer see 405 when the exact and param path are defined with different HTTP methods. #3330 #3340
- You no longer see `Address family not supported by protocol` or `Connection refused` error anymore. #3425
- You no longer see escaped Unicode from a JSON response in <type://DocService>. #3396
- You no longer see the wrong response body when the payload violates the protocol or is too large. #3419
- You can now use the Thrift client and service that is generated by `java:fullcamel` Thrift generator.
  #3269 #3360 #3369
