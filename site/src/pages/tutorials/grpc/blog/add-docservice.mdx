...
Take advantage of Armeria's Documentation service which automatically creates documentation of your service methods, as well as providing means to test out the methods.
...
You need to have the all the files you created for the server-side. No client files are required for this step.
...
## 1. Create DocService instance

1. Create a <type://DocService> instance in the Main.java.
...
```
2. Enable unframed requests by adding `.enableUnframedRequests(true)` to the GrpcService builder. This makes the blog service to support requests that are not framed in gRPC wire format. Thus enabling you to test out calling service methods from the Documentation service.
```java filename=Main.java
private static Server newServer(int port) throws Exception {
...
1. Click the URL `http://127.0.0.1:8080/docs` from the log message or open the URL on a web browser. If you see the Document service page, you've launched the <type://DocService> and server successfully.

![](../../../../images/tutorial_blogservice_grpc_docservice_start.png)

2. Click the **CreateBlogPost()** method link in the left panel. You can make calls to the creation method from the page opened.
...
In this step, we've added the Documentation service to a server. We've come to the end of this tutorial. Next, try adding more service methods to the tutorial or have a go at developing a service of your own.
