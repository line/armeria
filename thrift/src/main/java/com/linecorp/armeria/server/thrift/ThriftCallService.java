...
public final class ThriftCallService implements RpcService {
     ...
     */
    public static ThriftCallService of(Map<String, List<?>> implementations) {
        return new ThriftCallService(implementations);
    }

    private final Map<String, List<ThriftServiceEntry>> entries;

    public ThriftCallService(Map<String, List<?>> implementations) {
        ...
        entries = implementations.entrySet().stream().collect(
                toImmutableMap(Map.Entry::getKey, toThriftServiceEntries()));
    }

    private static Function<Entry<String, List<?>>, List<ThriftServiceEntry>> toThriftServiceEntries() {
        return entry -> {
            final String name = entry.getKey();
            final List<?> implementations = entry.getValue();
            return implementations
                    .stream()
                    .map(implementation -> new ThriftServiceEntry(name, implementation))
    ...
    @Nullable
    public ThriftServiceEntry getFirstService(String name) {
        final List<ThriftServiceEntry> thriftServiceEntries = entries.get(name);
        if (thriftServiceEntries == null || thriftServiceEntries.isEmpty()) {
            return null;
        }
        return thriftServiceEntries.get(0);
    }
    ...
    public RpcResponse serve(ServiceRequestContext ctx, RpcRequest call) throws Exception {
        ...
        final ThriftServiceEntry e = getFirstService(serviceName);
        if (e != null) {
