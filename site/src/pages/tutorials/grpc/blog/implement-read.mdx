...
Retrieving a single blog post requires a post ID. We haven't saved any post ID returned by the server. (Although we very much know that our tutorial service issues incremental ID.). To check IDs, we'll use Armeria's <type://LoggingClient> to decorate the our blog client.

<Tip>

  To use an interceptor instead, use <type://GrpcServiceBuilder#intercept()>.
