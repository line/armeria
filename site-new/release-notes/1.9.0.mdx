...
- A new module `armeria-graphql` integrates with [GraphQL Java](https://www.graphql-java.com/) to let you serve
  ...
  :::
- You can now transform the objects of [HttpRequest](type) and [HttpResponse](type) using a function.
  #3606 #3624
  ```java
  HttpRequest req1 = ...;
  HttpRequest req2 = 
  ...
  HttpResponse res2 = 
      res1.mapHeaders(headers -> headers.toBuilder()
          ...
          .mapTrailers(trailers -> trailers.toBuilder()
                                           ...
                                           .build());
  ```
- You can now collect the all elements of a [StreamMessage](type) with [StreamMessage#collect()](type). #3603
  ```java
  ...
  ```
- You can exclude a [Route](type) from another [Route](type) when configuring a service. #2737 #3562
  ```java
  ServerBuilder sb = ...;
  sb.route()
    .pathPrefix("/api")
  ...
  ```
- You can now update the service bindings without restarting a [Server](type) using
  [Server#reconfigure(ServerConfigurator)](type). #3041 #3362 Note that there are some limitations on 
  the [Server](type) reconfiguration. Please check out #3041 for details.
  ```java
  Server server = ...;
  ...
  ```
- You can now use [RequestOptions](type) to configure various options at request level. #3593
  ```java
  HttpRequest req = ...;
  WebClient client = ...;
  ...
  client.prepare()
        ...
        .execute();
  ```
- You can now fluently build a [BlockingTaskExecutor](type) using [BlockingTaskExecutorBuilder](type).
  #3315 #3389
  ```java
  BlockingTaskExecutor.builder()
                      ...
                      .build();
  ```
- You can now easily get a boolean value from [QueryParams](type) and [HttpHeaders](type). #3600
  ```java
...
- You can now conveniently configure a loopback port with
  [ServerBuilder#localPort(int,Iterable)](type). #3599 #3601
  ```java
  Server.builder()
        .localPort(8080)
...
- All Jackson modules in the class loader are registered automatically by default via Jackson's
  ...
  - You can override this behavior by registering your [JacksonModuleProvider](type) implementation
...
- You can now get the last value of a header or a query parameter using
  [HttpHeaders#getLast(CharSequence)](type) and [QueryParams#getLast(String)](type). #3438 #3568
  ```java
  HttpHeaders headers =
      HttpHeaders.builder()
  ...
  ```
- [ContentTooLargeException](type) now provides you the following 3 properties about why
...
- You can now convert a [Multipart](type) into an [HttpResponse](type). #3642
- You can now warm up an `EventLoopGroup` using [EventLoopGroups#warmUp(EventLoopGroup)](type). #3363 #3610
- You can now figure out the number of connections managed by a [ClientFactory](type) with
  [ClientFactory#numConnections()](type). #3596 #3613
- You can now specify the number of event loop threads and blocking task threads more conveniently. #3597 #3602
  ...
  ClientFactory.builder()
               ...
               .build();
  ```
- You can now clear the mocking state of a [MockWebServerExtension](type) using
  [MockWebServerExtension#reset()](type). #3629

{/* truncate */}

## 📈 Improvements

- Various optimizations for [StreamMessage](type) implementations. #3585 #3584
- You can now use an optimized context-aware `CompletableFuture` for Java 12 and above. #2119 #3615
- Various multipart MIME types were added to [MediaType](type). #3642

## 🛠️ Bug fixes

- `ByteBuf`s are no longer leaked when using:
  - Spring WebFlux integration with compression #3608 #3617; or
  - [StreamMessageDuplicator](type) #3476 #3583
- You no longer see a [ResponseTimeoutException](type) with [CircuitBreakerClient](type) when a large number
  of messages are received from a response. #3631
- You can now set the service name of an annotated service using
  the [ServerBuilder#defaultServiceNaming(ServiceNaming)](type). #3589
- You no longer see a [ClosedStreamException](type) when an [EndpointGroup](type) is empty. #3381 #3636
- Late `Subscriber`s correctly run on the specified executor. #3623 #3632
- An exception raised in [THttpService](type) is now properly peeled using
  [Exceptions#peel(Throwable)](type). #3588
- You no longer see an `UnsatisfiedLinkError` when a Netty JAR without native libraries is included. #3485 #3607
- Additional headers are now correctly updated to [RequestOnlyLog#requestHeaders()](type). #3579 #3581
- A timeout is more precisely scheduled for a request or a response. #3570
- You no longer see a duplicate `Content-Type` header when using [Multipart](type). #3642
