...
- **Outlier Detection for Connections**: You can now use [OutlierDetection](type) to detect unhealthy
...
- **Automatic value loader**: You can now use an [AsyncLoader](type) to (re)load a value atomically, lazily and
...
- **Configurable Response Timeout**: You can now use [ResponseTimeoutMode](type) to configure when a response
  ...
  ClientFactory
    ...
    .build();
  ```
- **Fallback File Extensions**: You can now set fallback file extensions in [FileService](type).
...
- **Sending Arbitrary Content in gRPC-JSON Transcoding**: You can now use [google.api.HttpBody](https://github.com/googleapis/googleapis/blob/master/google/api/httpbody.proto)
  ...
  ```
- **Zone-Aware Load Balancing**: [XdsEndpointGroup](type) now supports [zone-aware](https://www.envoyproxy.io/docs/envoy/latest/intro/arch_overview/upstream/load_balancing/zone_aware)
  load balancing which prioritizes sending traffic to local zones. #5785 #5808
- **WebSocketService Testing**: You can now use [ServerExtension#webSocketClient()](type) to test [WebSocketService](type). #5538 #5765
...
- [ClientRequestContext#cancel()](type) now cancels the associated request immediately. #5800
- The health checking behavior of [XdsEndpointGroup](type) is more aligned with Envoy. #5785
- [Subsets](https://www.envoyproxy.io/docs/envoy/latest/intro/arch_overview/upstream/load_balancing/subsets#load-balancer-subsets) now
  supports features available in the default load balancer of [XdsEndpointGroup](type) #5801
- New headers from Guava have been added to [HttpHeaderNames](type). #5789
...
- `ResteasyClient` now correctly releases [ClientFactory](type) when it is closed. #5833
- Content decompression exceptions are now properly propagated. #5177 #5620
- [@Blocking](type) now correctly works with gRPC-Kotlin stubs #5295 #5399
- Fixed [DocService](type) sidebar to occupy at most 40% of the window width. #5091 #5839
- `InputStream` created from [StreamMessage#toInputStream(Function)](type) now correctly raises
  an exception if the original [StreamMessage](type) is closed with an exception. #5859
- [ClientRequestContext#isTimedOut()](type) now correctly returns `true` if a DNS query, a session creation or
...
- Modified the behavior of [Endpoint#withAttrs(Attributes)](type) to merge the original [Attributes](type)
  with the new one. #5802
  - Instead, [Endpoint#replaceAttrs(Attributes)](type) is added to replace the entire attributes of [Endpoint](type).
