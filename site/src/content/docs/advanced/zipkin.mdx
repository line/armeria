...
Please note that we specified [RequestContextCurrentTraceContext](type). It stores the trace context into a
[RequestContext](type) and loads the trace context from the [RequestContext](type) automatically. Because of that,
...
Now, you can specify [BraveService](type) using [Decorating a service](/docs/server/decorator) with the
...
create [BraveClient](type) and [BraveService](type). Otherwise, there might be problems if the instances are not
