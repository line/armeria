...
- **xDS-based EndpointGroup**: You can utilize [xDS protocol](https://www.envoyproxy.io/docs/envoy/latest/api-docs/xds_protocol)
  to create [XdsEndpointGroup](type) for service discovery. #5342 #5336
  ```java
...
- **Service-specific EventLoopGroup**: You can now assign a different `EventLoopGroup` to each service for
  ...
  Server
    ...
    .build();
  ```
- **Thrift Service Decorators**: You can now use [@Decorator](type) annotations to conveniently apply
...
- **Sensitive Header Masking**: You can now easily mask request and response headers
  using [HeadersSanitizer](type). #5175 #5188
...
- **Custom Exception Handling for gRPC clients**: You can customize how your gRPC clients respond to exceptions
  by implementing [GrpcExceptionHandlerFunction](type). #5347 #5351
  ```java
  GrpcClients
...
- **WebSocket Service Fallback**: You can now set up a fallback service for
  [WebSocketService](type) to handle both WebSocket and non-WebSocket requests on the same path with
  [WebSocketServiceBuilder#fallbackService(HttpService)](type) #5368
  ```java
...
- **WebSocket Frame Aggregation**: You can now enable to aggregate WebSocket continuation frames for simplified
  message processing using [WebSocketServiceBuilder#aggregateContinuation(boolean)](type). #5357
  ```java
  WebSocketService
    .builder()
...
- **Active Connection Monitoring**: You can monitor the number of active connections with
  ...
  [ConnectionPoolListener#metricCollecting(MeterRegistry)](type) #4277 #5288
...
- **Graceful Executor Shutdown**: You can now gracefully terminate a custom `ExecutorService` in sync with the
  [Server](type)'s shutdown procedure. #5031 #5087
  ```java
  ServerListener
    .builder()
    ...
    .build()
  ```
- **Session Protocol in RoutingContext**: You can now acquire [SessionProtocol](type) directly from
  [RoutingContext#sessionProtocol()](type) for enhanced routing and [ExchangeType](type) decisions.
...
- Annotated services now support [HttpResult](type) wrapped in RxJava objects. #5380 #5386
- You can now create [HttpFile](type) with
...
- [WebSocketService](type) is restructured as an interface, allowing for custom implementations. #5368
- Introduce [PreTlsHandshakeException](type) to better report exceptions occurring before a TLS handshake
  initiates. #5344
- Error messages for gRPC client stub generation have been clarified. #5387

## 🛠️ Bug fixes

- The completions of [ClientRequestContext#whenResponseCancelling()](type) and [ClientRequestContext#whenResponseCancelled()](type)
  are now guaranteed even if [RequestContext#cancel()](type) is invoked before the initialization of
  the timeout scheduler. #5212
- Fixed a leak within [WebSocketClient](type) when an invalid response is received. #5412
...
- [CloseWebSocketFrame](type) with empty reason phrases are now correctly handled. #5388
- [Flags](type) now correctly logs all messages when [RequestContextExportingAppender](type) is configured in
  `logback.xml`. #5327 #5361
- You no longer see [EndpointSelectionTimeoutException](type) when [HealthCheckedEndpointGroup](type) is
  initialized with duplicate [Endpoint](typeplural). #5343
- [HttpHeaders#containsBoolean(CharSequence,boolean)](type) now checks all header entries with the same
  name. #5319
- [GrpcService](type) provides more consistent `ServerCall.Listener` behavior under error conditions. #5273

## 📃 Documentation

- Updated [Production Checklist](/docs/advanced/production-checklist) with guidance on
  using [ClientFactoryBuilder#maxNumEventLoopsPerEndpoint(int)](type). This addition will help you mitigate
