...
Note that you can omit the value of [@Param](type) if you compiled your Kotlin code with [`java-parameters`](https://kotlinlang.org/docs/reference/compiler-reference.html#java-parameters) option.
...
[ServiceRequestContext](type) is injected to the default `CoroutineContext` of a `suspend` function,
so you can access [ServiceRequestContext](type) from a `suspend` function with [ServiceRequestContext.current()/](type) 
...
[@Blocking](type) or enable [AnnotatedServiceBindingBuilder#useBlockingTaskExecutor(boolean)](type).
...
coroutines should be executed in a thread that is aware of [RequestContext](type).
...
using [CoroutineContextService](type) decorator.
...
## Coroutines support for [RestClient](type)

Armeria provides extension methods to convert [RestClientPreparation#execute(Class)](type) to a `suspend` function.
...
You can use [GrpcClients](type) to specify a coroutine stub that ends with `CoroutineStub` when creating a 
gRPC-Kotlin client.
...
[ServiceRequestContext](type) and an event loop `CoroutineDispatcher`. You can access 
[ServiceRequestContext](type) from the suspend functions with [ServiceRequestContext#current()](type), 
...
gRPC-Kotlin service. You can create your own `CoroutineContext` using [CoroutineContextProvider](type), and 
add it to your service through Java SPI.

Note that the [ServiceRequestContext](type) is still a part of the 
