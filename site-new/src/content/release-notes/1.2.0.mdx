...
- You can now bind a [GrpcService](type) to a custom route path. #3072 #3083
  ...
  sb.service(GrpcService.builder()
                        ...
                        .build());
  ```
- You can now use [DecodingService](type) for decoding a compressed request. #1521 #3087
- You can now use [HttpResponse#ofRedirect(String)](type) for easily sending a redirect response. #914 #3059
  ```java
...
- You can now choose a path when a Thrift/gRPC service has more than one path
  in the [DocService](type). #2167 #3024
- You can now cancel a service's request or client's response with [CancellationException](type) or
  any `Throwable` using [RequestContext#cancel()](type) or
  [RequestContext#cancel(Throwable)?full](type). #3046 #3080
...
- You can now use milliseconds as an interval unit in [EurekaEndpointGroup](type). #3108
...
- [BraveClient](type) and [BraveService](type) do not override
...
- You no longer see an outdated JS bundle in the browser
  when the [DocService](type) page is rebuilt. #3094

## 🏚️ Deprecations

- The following methods have been deprecated in favor of their `millis`-accepting versions: #3108
  - [EurekaEndpointGroupBuilder.registryFetchIntervalSeconds()](type)
  - [EurekaUpdatingServerListener.leaseDurationSeconds()](type)
  - [EurekaUpdatingServerListener.renewalIntervalSeconds()](type)
