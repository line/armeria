...
- You can now repeatedly aggregate an <type://HttpRequest> and <type://HttpResponse> when using
  <type://HttpRequest#aggregate()> and <type://HttpResponse#aggregate()>. If an <type://HttpRequest> and
  <type://HttpResponse> is aggregated before, the cached value is returned for the next calls. #3789 #4366
  ```java
  HttpRequest request = ...;
  ...
  ```
- You can now customize the behavior of auto refresh for `AddressResolverGroup` with
...
- You can now export <type://RequestOnlyLog#requestCause()> and <type://RequestLog#responseCause()> to MDC
  by specifying `req.cause` and `res.cause` to an appender of Logback.
  ```xml
...
- You can now trace a <type://RequestContext> leak by specifying `-Dcom.linecorp.armeria.requestContextLeakDetectionSampler=<sampler-spec>`
  ...
  ```
- <type://DocService> now automatically generates a specification for a JSON object or a protobuf message used 
  in an annotated service's parameter or return type. #4309 #4322
- You can now upload files to your <type://GraphqlService> when using 
  [GraphQL multipart requests](https://github.com/jaydenseric/graphql-multipart-request-spec). #3971 #4199
- You can now use `armeria-version-catalog` module to easily import Armeria dependencies
...
- `CompositeException` now shows full stack traces for all nested exceptions 
...
- The method signature of <type://ResponseConverterFunctionProvider#createResponseConverterFunction(Type)> has
  been changed.
  - <type://ResponseConverterFunctionProvider> no longer takes a <typeplural://ResponseConverterFunction> as 
    the second parameter. Use <type://DelegatingResponseConverterFunctionProvider> to create a new 
    <type://ResponseConverterFunction> based on the delegating <typeplural://ResponseConverterFunction>.
