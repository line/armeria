...
- You can now create a [StreamMessage](type) from a `Path` or a `File`. #3344
  ```java
  Path path = Paths.get("...");
  StreamMessage<HttpData> publisher = StreamMessage.of(path);
  ```
- You can now filter or transform values of a [StreamMessage](type) using
  [StreamMessage#filter(Predicate)](type) or [StreamMessage#map(Function)](type). #3351
...
- You can now convert an exception into an [RpcResponse](type) in [THttpService](type). #3349 #3383
  ```java
  ...
  ```
- You can now set a response timeout and attributes using [WebClientRequestPreparation](type). #3347 #3357
  ```java
  WebClient client = ...;
  client.prepare()
        .get("/my-service")
...
- You can now customize the success condition of a metric. #3404 #3410
  ...
  ```
- You can now fluently build a [DecodingClient](type) using the [DecodingClientBuilder](type). #3348 #3372
  ```java
  DecodingClient.builder()
                ...
                .newDecorator();
  ```
- You can now fluently build an [HttpRequest](type) with a `Publisher<HttpData>`. #3343
  ```java
...
- You can now customize the default service name of a [RequestLog](type). #3232 #3366
  ```java
  Server.builder()
...
- You can now check if the current request is matched by any routes or not. #3365 #3378
  ...
  ```
- [BraveService](type) does not trace requests for [TransientService](type) such as
  [HealthCheckService](type) anymore. #3382
  - You should specify [TransientServiceOption#WITH_TRACING](type) if you want to trace them.
  ```java
  HealthCheckService.builder()
                    ...
                    .build();
  ```
- You can now clean up resources by overriding [FilteredStreamMessage#onCancellation(Subscriber)](type)
...
- You can now use `req.root_id` [BuiltInProperty](type) to log the ID of [RequestContext#root()](type).
  #3429 #3433
...
- You can now use OAuth 2.0 related features with the new `armeria-oauth2` module. #2268 #2840
  - [OAuth 2.0 server-side token authorization](https://datatracker.ietf.org/doc/rfc7662/) using
    [OAuth2TokenIntrospectionAuthorizer](type)
  - [Client credential grants](https://datatracker.ietf.org/doc/html/rfc6749#section-4.4) using
    [OAuth2ClientCredentialsGrant](type)
  - [Resource Owner Password Credentials Grant](https://datatracker.ietf.org/doc/html/rfc6749#section-4.3)
    using [OAuth2ResourceOwnerPasswordCredentialsGrant](type)
  - [OAuth 2.0 Token Revocation](https://datatracker.ietf.org/doc/rfc7009/) using [TokenRevocation](type)
...
- You no longer see `NullPointerException` in [HttpResponseDecoder](type). #3036 #3407
...
- You no longer see `CancellationException` when an [HttpResponse](type) is fully consumed on server-side.
  #3387
- You no longer see [ClosedSteamException](type) in Jetty service when it fails to write to an
  [HttpResponse](type). `EofException` is raised instead. #3412
- You no longer see `IllegalStateException` when [RequestContextHooks](type) is enabled. #3441 #3442
...
- [ServiceRequestContext](type) is added to the parameter of `handleMessage` in
  [AbstractUnaryGrpcService](type). #3403
...
- [GrpcStatusFunction](type) now extends `BiFunction<Throwable, Metadata, Status>`. #3329
- The `numberSteps` method in [WeightRampingUpStrategyBuilder](type) is now changed to `totalSteps`. #3377
