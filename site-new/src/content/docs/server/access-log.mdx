...
Access logging is disabled by default. If you want to enable it, you need to specify an [AccessLogWriter](type).
...
You can have different [AccessLogWriter](typeplural) for [VirtualHost](typeplural) and [Service](typeplural).
...
| `%t`                                        | No                | the date, time and time zone that the request was received, by default in `strftime` format %d/%b/%Y:%H:%M:%S %z. (for example, `10/Oct/2000:13:55:36 -0700`) Refer to [Customizing timestamp format](#customizing-timestamp-format) for more information.                                                                                                                                                                                                                                                    |
...
| `%{REQUEST_LOG_NAME}L`                      | Yes               | the value of the specified variable of the [RequestLog](type). Refer to [Retrieving values from RequestLog](#retrieving-values-from-requestlog) for more information.                                                                                                                                                                                                                                                                                                                                         |
...
[RequestLog](type) holds information about the request, so a user may want to write these values to his or
...
You can specify your own log writer which implements a `Consumer` of [RequestLog](type).
...
Alternatively, you can specify your own mapper or logger for a [VirtualHost](type), e.g.
...
You can also specify your own logger for the specific [VirtualHost](type). In this case, the mapper or logger
you set for a specific [VirtualHost](type) will override the access logger set via
[ServerBuilder#accessLogger(Logger)](type).
