...
import com.linecorp.armeria.internal.Writability;
...
public final class ArmeriaHttpUtil {
    ...
    /**
     * Writes the specified inbound {@code data} to the specified {@code writer}. This utility method accounts
     * for the number of the bytes that are waiting to be consumed by a {@link Subscriber} via the specified
     * {@link Writability}. When the {@link Subscriber} does not consume fast enough,
     * {@link Writability#isWritable()} will start to return {@code false}, and this method will suspend
     * the inbound traffic of the related socket {@link Channel}. Likewise, when the {@link Subscriber}
     * consumes enough amount of data, the {@link Writability#isWritable()} will start to return {@code true},
     * and then this method will resume the inbound traffic.
     */
    public static void publishInboundTraffic(
            ChannelHandlerContext ctx, Writer<HttpObject> writer, ByteBuf data, Writability writability) {

        final byte[] dataArray = new byte[data.readableBytes()];
        data.getBytes(data.readerIndex(), dataArray);
        writer.write(HttpData.of(dataArray));
            ...
            writer.awaitDemand()
                  .whenComplete((res, cause) -> {
                      ...
                      try {
                          ...
                          Exceptions.logIfUnexpected(logger, ch, t);
                      }
                  });
        }
    }
