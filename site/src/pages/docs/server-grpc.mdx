...
There are two ways to decorate an individual gRPC service:

- bind a decorator using the builder method of <type://GrpcService>
...
- bind a decorator using <type://@Decorator>

```java
@Decorator(ClassDecorator.class)
class MyHelloServiceImpl extends MyHelloServiceImplBase {
...
If you use the above at the same time, the decorators will be applied in the order of
`FirstDecorator`, `SecondDecorator`, `ClassDecorator`, and `MethodDecorator`.
