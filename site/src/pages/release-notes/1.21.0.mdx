...
- You can now set the default <type://HttpHeaders> to <type://Server>, <type://VirtualHost> and
  ...
  ```
- You can now set <type://Endpoint#host()> and `:authority` or `Host` header
...
- <type://HttpHeaders> set by <type://ClientBuilder#setHeader(CharSequence,Object)>,
  <type://ClientBuilder#setHeader(CharSequence,Object)> and their variants are now stored in
  <type://ClientRequestContext#defaultRequestHeaders()> which have the lowest priority and don't override the
  values in <type://RequestHeaders>.
  - Previously, the <type://HttpHeaders> set via <type://ClientBuilder> were stored in
    <type://ClientRequestContext#additionalRequestHeaders()> which may override the values in
    <type://RequestHeaders> #4440 #4520
- Fixed a regression that `void` methods returns `200 OK` instead of `204 NoContent` if <type://@Produces> 
