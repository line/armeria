...
final class HttpServerHandler extends ChannelInboundHandlerAdapter implements HttpServer {
    ...
    private static final Set<HttpMethod> ALLOWED_METHODS =
            ImmutableSet.<HttpMethod>builder()
                    ...
                    .add(HttpMethod.POST)
                    .add(HttpMethod.PUT)
                    .add(HttpMethod.TRACE).build();

    private static final String ALLOWED_METHODS_STRING =
            String.join(",", ALLOWED_METHODS.stream().map(HttpMethod::name).collect(Collectors.toList()));
    ...
    public void channelInactive(ChannelHandlerContext ctx) throws Exception {
        final HttpObjectEncoder responseEncoder = this.responseEncoder;
    ...
    private void handleRequest(ChannelHandlerContext ctx, DecodedHttpRequest req) throws Exception {
        ...
        unfinishedRequests++;
        res.awaitClose().whenComplete((ret, cause) -> {
            req.abort();
            if (cause == null) {
                reqLogBuilder.end();
            } else {
        ...
        res.subscribe(new HttpResponseSubscriber(ctx, responseEncoder, reqCtx, req),
                      eventLoop);
    }
