...
final class SecretStream extends RefCountedStream<SecretSnapshot> {
    ...
    @Override
    protected Subscription onStart(SnapshotWatcher<SecretSnapshot> watcher) {
        if (secret != null) {
            final SecretXdsResource resource = new SecretXdsResource(secret);
            return SnapshotStream.just(new SecretSnapshot(resource)).subscribe(watcher);
        }
        ...
        if (sdsSecretConfig.hasSdsConfig()) {
            ...
            if (configSource == null) {
                final SnapshotStream<SecretSnapshot> errorStream = SnapshotStream.error(
                        new XdsResourceException(XdsType.SECRET, name, "config source not found"));
                return errorStream.subscribe(watcher);
            }
            final SnapshotStream<SecretSnapshot> resourceNodeAdapter =
                    new ResourceNodeAdapter<SecretXdsResource>(configSource, context, name, XdsType.SECRET)
                            .map(SecretSnapshot::new);
            return resourceNodeAdapter.subscribe(watcher);
        }
        final SecretXdsResource resource = context.bootstrapSecrets().get(name);
        emit(new SecretSnapshot(resource), null);
        return () -> {};
    }
}
