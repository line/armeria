...
In the earlier step, we tried creating a blog post.
...
And here, we'll throw a custom exception and add a `BlogServiceExceptionHandler` for handling.
...
First, add an exception and an exception handler class for the exception to be thrown when there is no blog post for the given ID.

1. Add an exception in the `blog.thrift` file.
...
3. Add an exception handler class for the blog service.
  ...
  ```
3. In the `Main` class, bind the `BlogServiceExceptionHandler` to our service.
...
1. Update the `blog.thrift` file by adding a method and a struct.
  ```thrift filename=blog.thrift
  /**
  ...
  ```
2. [Compile the thrift file](/tutorials/thrift/blog/create-service#2-compile-the-thrift-file) to generate Java code.
3. Add a service method in the `BlogServiceImpl` class to retrieve a single post.
  ...
  ```
  4. Throw an exception in case there is no blog post for the given ID.
  ```java filename=BlogServiceImpl.java
  import example.armeria.blog.thrift.BlogNotFoundException;
...
1. Update the `blog.thrift` file by adding a method and structs.
  ```thrift filename=blog.thrift
  /**
  ...
  ```
2. [Compile the thrift file](/tutorials/thrift/blog/create-service#2-compile-the-thrift-file) to generate Java code.
3. Add a service method in the `BlogServiceImpl` class to retrieve multiple posts.
  ```java filename=BlogServiceImpl.java
  ...
  public class BlogServiceImpl implements BlogService.AsyncIface {
    ...
    public void listBlogPosts(ListBlogPostsRequest request,
      ...
      final List<BlogPost> blogPosts = blogPosts = this.blogPosts.values().stream().collect(Collectors.toList());
      resultHandler.onComplete(new ListBlogPostsResponse().setBlogs(blogPosts));
    }
  }
  ```
4. Sort the blog posts based on the value of the `descending` parameter.
  ```java filename=BlogServiceImpl.java
...
This time, we'll implement the client-side for reading blog posts. First, let's add an exception  in `blog.thrift` file.
...
Now, let's implement a client method for each corresponding server method.
...
1. Update the `blog.thrift` file by adding a method and a struct.
  ...
  ```
2. [Compile the thrift file](/tutorials/thrift/blog/create-service#2-compile-the-thrift-file) to generate Java code.
3. Add a method in the `BlogClient` class to retrieve a single post.
...
1. Update the `blog.thrift` file by adding a method and structs.
  ...
  ```
2. [Compile the thrift file](/tutorials/thrift/blog/create-service#2-compile-the-thrift-file) to generate Java code.
3. Add a method in the `BlogClient` class to retrieve a list of posts.
  ```java filename=BlogClient.java
  import example.armeria.blog.thrift.ListBlogPostsRequest;
  import example.armeria.blog.thrift.ListBlogPostsResponse;
...
1. Call the `listBlogPosts()` method in the `testRun()` method.
  ```java filename=BlogClient.java
  void testRun() {
...
Since we haven't added any log messages, we can't tell if we've successfully retrieved blog posts or not.
To add log messages, we'll add a decorator in the following step.
Not only can we see the blog posts retrieved, we can obtain IDs to retrieve a post later on.

## 4. Decorate the client

Retrieving a single blog post requires a post ID.
We haven't saved any post ID returned by the server. (Although we very much know that our tutorial service issues incremental ID.).
To check IDs, we'll decorate the client with Armeria's <type://LoggingClient>.
Likewise, you can decorate your service also.
To decorate your service, use the [server decorator](https://armeria.dev/docs/server-decorator) or annotate your Apache Thrift class with <type://@LoggingDecorator>.

<Tip>

  Another option available is interceptor.
  See <type://ThriftClientBuilder> for information.

</Tip>

1. In the `main()` method, add a decorator to the client instance.
  ```java filename=BlogClient.java
...
2. Set up log messages:
    1. Add a dependency in the `build.gradle` file.
      ```groovy filename=build.gradle
      dependencies {
        implementation 'ch.qos.logback:logback-classic:1.2.10'
      }
      ```
    2. Add the logback.xml file in the `{project_root}/src/main/resources` folder.
      ...
      ```
3. Re-run the server and run the client.
  If succeed, you'll see an output like the following.
  ```bash
...
1. Call the `getBlogPost()` method with ID `1` for the second blog post.
  ```java filename=BlogClient.java
  void testRun() {
    createBlogPost("My first blog", "Yay");
    createBlogPost("Another blog post", "Creating a post via createBlogPost().");
...
2. Re-run the server and run the client.
  If succeed, you'll see an output like the following.
  ```bash
  ...
  ```
3. Change the argument to `100` and run the client again.
  Let's see what happens if you retrieve a blog post that does not exist.
  ```java filename=BlogClient.java
  void testRun() {
    createBlogPost("My first blog", "Yay");
    createBlogPost("Another blog post", "Creating a post via createBlogPost().");
...
Here, we've implemented service methods and client methods to retrieve blog posts.
We have also added a decorator. See [Decorating a client](/docs/client-decorator/) for more information.
Next, we'll implement a method for [updating a blog post](/tutorials/thrift/blog/implement-update).
