...
```
- You can specify allowed CORS origins via regular expression or `Predicate<String>`. #4982
```java
...
```
- You can flatten multiple <typeplural://StreamMessage> using <type://StreamMessage#flatMap()>. #3966 #4227
- <type://ClientFactory> can be closed with a grace period before closing HTTP/2 connections. #5489
...
- OAuth 2.0 modules have been refactored #5476
  - You can now easily implement your own [OAuth 2.0 Extension Grants](https://datatracker.ietf.org/doc/html/rfc6749#section-4.5) by extending `AccessTokenRequest`.
...
- <type://ServerBuilder#hostname(String)> returns a cached <type://VirtualHostBuilder> if available. #5418
```java
assert Server.builder().virtualHost("a.com") == Server.builder().virtualHost("a.com");
```
- You can add custom `details` when throwing an `ArmeriaStatusException` in a <type://AbstractUnsafeUnaryGrpcService>. #4306 #4341
...
- <type://XdsBootstraps> now supports `SelfConfigSource`. #5459
- <type://AccessLogWriter#log> is now called with the `RequestContext` set on the calling thread. #5500
- You can now pass the <type://RequestOnlyLog#authenticatedUser> to Logback's MDC easily using `authenticated.user`. #5499
...
- Armeria server no longer logs a duplicate route warning for routes with arbitrary parameter or header predicates. #5469
...
- <type://WebsocketFrame#ofPong(byte[])> now correctly sends a `PONG` Frame. #5533
- The `Complete` message in the GraphQL over WebSocket Protocol is now constructed securely. #5531
- <type://ClientFactory> no longer throws an exception intermittently when shutting down. #5494 #5524
...
- DocService now also supports JSON streaming requests. #5218 #5381

## ☢️ Breaking changes

- The `armeria-brave` module has been fully removed. #5453
  - See the [brave6 release notes](https://github.com/openzipkin/brave/releases/tag/6.0.0) for more information.
...
- Server-side HTTP1.1 request headers are converted to Armeria headers with zero-copy. #4853 #4863
...
- <type://ServerBuilder#virtualHost(String,String)>, <type://ServerBuilder#withVirtualHost(Consumer)>,
  <type://VirtualHostBuilder#hostname(String)> have been deprecated as they don't support <type://VirtualHostBuilder> re-usability.
