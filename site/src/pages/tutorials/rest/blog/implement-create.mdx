...
You must have the following files ready for creating a blog post. You can always [download](https://github.com/line/armeria-examples/blob/main/tutorials/rest-api-annotated-service/src/main/java/example/armeria/server/blog/) the full version, instead of creating one yourself.
...
Let's use Armeria's <type://ServerExtension>.
This approach will automatically set-up and tear-down a server before tests are run.
This is more efficient than directly connecting to the server, as it eliminates the need to execute the main method  to set up a server before running our tests.
...
We've also written a test method and registered <type://ServerExtension> to our test.
