...
- **xDS-based EndpointGroup**: You can utilize xDS protocols to create <type://XdsEndpointGroup> for service
  discovery. #5342 #5336
  ```java
...
- **Custom Exception Handling for gRPC clients**: You can customize how your gRPC clients respond to exceptions
  ...
  ```
- **Thrift Service Decorators**: You can now utilize <type://@Decorator> annotations to conveniently apply
  custom logic to Thrift services. #4230 #5356
  ```java
...
- **WebSocket Service Fallback**: Now, it is possible to set up a fallback service for 
- <type://WebSocketService> to handle non-WebSocket requests seamlessly with
  <type://WebSocketServiceBuilder#fallbackService(HttpService)> #5368
  ```java
  WebSocketService webSocketService = 
    WebSocketService.builder()
...
- message processing using <type://WebSocketServiceBuilder#aggregateContinuation(boolean)>. #5357
  ...
  WebSocketService
    ...
    .build(); 
  ```
- **Active Connection Monitoring**: You can track the number of active connections with 
  ...
  ClientFactory clientFactory =
    ClientFactory
      .builder()
      .connectionPoolListener(ConnectionPoolListener.metricCollecting(meterRegistry))
      .build();
  WebClient
    .builder("https://my-backend.com")
    .factory(clientFactory)
    .build();
  ```
- **Graceful Executor Shutdown**: You can now gracefully terminate a custom `ExecutorService` in sync with the
- <type://Server>'s shutdown procedure using <type://ServerListenerBuilder#shutdownWhenStopped(ExecutorService,Duration)>
  and <type://ServerListenerBuilder#shutdownWhenStopped(ExecutorService,Duration)> #5031 #5087
  ```java
  ServerListener
    .builder()
...
- **Session Protocol in RoutingContext**: You can now acquire <type://SessionProtocol> directly from
  <type://RoutingContext#sessionProtocol()> for enhanced routing and <type://EchangeType> decisions.
- You can now easily mask request and response headers with ... #5175 #5188
  - TODO(ikhoon): TBU
- **Eureka Health Check Customization**: You can override the default health check URL when registering with 
  Eureka. #5369
  ```java
...
- <type://ClientRequestContext#whenResponseCancelling()> and <type://ClientRequestContext#whenResponseCancelled()>
  is now correctly completed even if the  `CancellationScheduler` has not been initialized #5212
- Fixed a leak within a WebSocket client when an invalid response is received. #5412
- `json_name` attribute in protobuf files is now correctly used for matching query parameters during
  gRPC HTTP/JSON transcoding. #5193 #5366
- Implicit request objects are now correctly converted. #5389 #5393
- Fixed an issue with handling an empty reason phase in WebSocket Close frames. #5388
- <type://Flags> logging is now consistent, irrespective of <type://RequestContextExportingAppender> 
  configurations in `logback.xml`. #5327 #5361
- You no longer see <type://EndpointSelectionTimeoutException> when <type://HealthCheckedEndpointGroup> is 
  initialized with duplicate `Endpoint`s. #5343
- <type://HttpHeaders#containsBoolean(CharSequence,boolean)> now checks across all header entries with the same
  name. #5319
...
- Envoy control plain 1.0.42 (new)
