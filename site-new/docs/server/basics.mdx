...
To start a server, you need to build it first. Use [ServerBuilder](type):
...
on a request path respectively. [ServerBuilder](type) also provides advanced path mapping such as regex and
glob pattern matching.

Also, we decorated the second service using [LoggingService](type), which logs all requests and responses.
You might be interested in decorating a service using other decorators, for example to gather metrics.

You can also use an arbitrary object that's annotated by the [@Path](type) annotation 
...
Use [ServerBuilder#virtualHost(String)](type) to configure [a name-based virtual host]:
...
you can use [ServiceRequestContext#clientAddress()](type). But you need to configure
your [ServerBuilder](type) before doing that:
