...
public class HeaderDateTimeFormatTest {
     ...
     */
    private static final long DATE = 784111777000L;

    @Test
    public void testParse() throws ParseException {
        final long parsedDateWithSingleDigitDay = parse("Sun, 6 Nov 1994 08:49:37 GMT");
        assertEquals(DATE, parsedDateWithSingleDigitDay);

        final long parsedDateWithDoubleDigitDay = parse("Sun, 06 Nov 1994 08:49:37 GMT");
        assertEquals(DATE, parsedDateWithDoubleDigitDay);

        final long parsedDateWithDashSeparatorSingleDigitDay = parse("Sunday, 06-Nov-94 08:49:37 GMT");
        assertEquals(DATE, parsedDateWithDashSeparatorSingleDigitDay);

        final long parsedDateWithSingleDoubleDigitDay = parse("Sunday, 6-Nov-94 08:49:37 GMT");
        assertEquals(DATE, parsedDateWithSingleDoubleDigitDay);

        final long parsedDateWithoutGMT = parse("Sun Nov 6 08:49:37 1994");
        assertEquals(DATE, parsedDateWithoutGMT);
    }

    @Test
    public void testFormat() {
        final String formatted = format(DATE);
