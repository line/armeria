...
final class HeaderDateTimeFormat {
    ...
    static long parse(String text) {
        final TemporalAccessor accessor = parse0(text);
        return accessor.getLong(ChronoField.INSTANT_SECONDS) * 1000L +
               accessor.getLong(ChronoField.NANO_OF_SECOND) / 1000000L;
    }

    private static TemporalAccessor parse0(String text) {
        requireNonNull(text);
        TemporalAccessor accessor;
        try {
            accessor = format1.parse(text);
        } catch (Exception e1) {
            // Try the second preferred format.
            try {
                accessor = format2.parse(text);
            } catch (Exception e2) {
                // Try the variant of the second preferred format.
                try {
                    accessor = format2a.parse(text);
                } catch (Exception e2a) {
                    // Try the third preferred format.
                    try {
                        accessor = format3.parse(text);
                    } catch (Exception e3) {
                        // None worked.
