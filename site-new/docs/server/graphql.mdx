...
Once you've finished the implementation of the [DataFetcher](https://javadoc.io/doc/com.graphql-java/graphql-java/latest/graphql/schema/DataFetcher.html), you need to build a [GraphqlService](type) using
a [GraphqlServiceBuilder](type) and add it to the [ServerBuilder](type):
...
[GraphqlServiceBuilder#useBlockingTaskExecutor(boolean)](type) to `true` so the service runs in
