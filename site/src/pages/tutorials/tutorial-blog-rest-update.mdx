...
To test updating a blog post, you need to have blog posts created, which requires you to have the creation method implemented in the BlogService.java. See [Step 4. Implement CREATE](/tutorials/tutorial-blog-rest-create) for instructions.
...
```java filename=BlogService.java highlight=7,10
...
1. Run the server like we did in [Step 1. Create a server](/tutorials/tutorial-blog-rest-create-server) by running the `main()` method or using Gradle. When you see the message, "Server has been started", you can try testing service methods.
...
Next, at [Step 7. Implement DELETE](/tutorials/tutorial-blog-rest-delete), we'll implement a DELETE operation to delete 
blog posts.
