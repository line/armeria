...
1. **Downstream HTTP filters** — xDS filters declared in the Listener's `http_filters`
   (see [Built-in filters](/docs/advanced/xds/extensions#built-in-filters) for available filters).
...
The following listener defines a custom downstream filter, a router with an upstream
filter, and two routes pointing to different clusters. See
[Built-in filters](/docs/advanced/xds/extensions#built-in-filters) for which
filters are bundled and how to register custom ones.
