...
To test making calls on the Documentation service, enable unframed requests by setting <type://GrpcServiceBuilder#enableUnframedRequests(boolean)> to true. This makes the blog service to support requests that are not framed in gRPC wire format. For us, this enables making calls on a web browser.
...
1. Click the URL `http://127.0.0.1:8080/docs` from the log message or open the URL on a web browser. If you see the Document service page, you've successfully launched the <type://DocService> and server.
