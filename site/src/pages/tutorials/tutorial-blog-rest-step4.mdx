...
3. Map these service methods with the HTTP GET method by adding the <type://@Get> annotation as follows.
4. Bind the endpoint `/blogs` to the method.

```java filename=BlogService.java highlight=1,7
@Get("/blogs")
public HttpResponse getBlogPost(int id) 
...
```java filename=BlogService.java highlight=2 showlineno=true
@Get("/blogs")
...
```java filename=BlogService.java highlight=5-12
...
```java filename=BlogService.java highlight=4-5
public HttpResponse getBlogPost(@Param int id)
    throws JsonProcessingException {
  ...
