...
- You can now check whether a [RequestContext](type) contains an attribute or not conveniently using
  [RequestContext#hasAttr(AttributeKey)](type) and [RequestContext#hasOwnAttr(AttributeKey)](type). #3014
- You can now specify the default service and log name programmatically for an annotated service. #2979
  - It was previously only possible to specify a service name with [@ServiceName](type) annotation.
  ```java
  Server.builder()
...
- Annotated services now support [Kotlin coroutines](https://kotlinlang.org/docs/reference/coroutines-overview.html).
  #2983 #3010 #3011
  - See [Kotlin coroutines support](/docs/advanced/kotlin) for more
    information.
  ```kotlin
  ...
  ```
- You can now get the current [ServiceRequestContext](type) from the Kotlin coroutine context when using
  [grpc-kotlin](https://github.com/grpc/grpc-kotlin). #3001
- You can now use [GrpcMeterIdPrefixFunction](type) to create meters with a `grpc-status` tag for gRPC clients
  ...
  Server.builder()
        ...
        .build();
  ```
- Added a new module `armeria-reactor3` that provides automatic [RequestContext](type) propagation for
  [Project Reactor](https://projectreactor.io/). #2999
  - See [the complete example](https://github.com/line/armeria-examples/tree/main/context-propagation/reactor)
...
- [RequestContext#setAttr(AttributeKey,Object)](type) now returns an old value being replaced. #2997 #3004
...
- You can now bind a [ServiceWithRoutes](type) (e.g. [GrpcService](type)) using the fluent builder API. #2996
...
- `localAddress()` and `remoteAddress()` builder methods in [ServiceRequestContextBuilder](type) and
  [ClientRequestContextBuilder](type) now accept `SocketAddress` instead of `InetSocketAddress`, so we have
...
- The default [MeterIdPrefixFunction](type) now always sets the `service` tag so that it doesn't violate the
  constraints enforced by some monitoring systems, e.g. Prometheus. #2977
- [TimeoutMode#SET_FROM_START](type) now schedules a timeout correctly. #2968
...
- Removed `HTTP_` or `Http` from various fields and methods.
  ...
  - [AbstractClientOptionsBuilder](type) and all its subtypes
...
- Removed redundant `add` prefix to some builder methods.
  - [RequestContextExporterBuilder](type)
    - `addBuiltIn()` → `builtIn()`
    - `addAttribute()` → `attr()`
    - `addRequestHeader()` → `requestHeader()`
    - `addResponseHeader()` → `responseHeader()`
    - `addKeyPattern()` → `keyPattern()`
  - [KeyStoreCredentialResolverBuilder](type)
    - `addKeyPassword()` → `keyPassword()`
    - `addKeyPasswords()` → `keyPasswords()`
- Renamed the methods in [DocService](type), [DocServiceBuilder](type) and their related classes.
...
- Renamed the methods in [ServerListenerBuilder](type).
...
- Renamed the methods in [EncodingServiceBuilder](type).
  - `encodableContentTypePredicate()` → `encodableContentTypes()`
  - `encodableRequestHeadersPredicate()` → `encodableRequestHeaders()`
- Renamed the methods in [TokenBucketThrottlingStrategyBuilder](type).
  - Removed the `with` from the builder methods.
    - `withMinimumBackoff()` → `minimumBackoff()`
    - `withHeadersScheme()` → `headersScheme()`
- [BasicToken#toHeaderValue()](type), [OAuth1aToken#toHeaderValue()](type) and [OAuth2Token#toHeaderValue()](type)
  have been renamed to `asHeaderValue()` for consistency. #3012
- [RequestLogProperty#NAME](type) is not always available for annotated service methods anymore. #2979
  - Use [RequestLog#whenComplete()](type) as usual.
- `RequestContext.setAttrIfAbsent()` and `computeAttrIfAbsent()` have been removed. #2997 #3004
  - Use [RequestContext#setAttr(AttributeKey,Object)](type) guarded by [RequestContext#hasOwnAttr(AttributeKey)](type).
