...
Use [FileService](type) to serve static files under a certain directory. [FileService](type) supports
...
You can configure [FileService](type) to generate the directory listing of the directories without
an `index.html` file using the `autoIndex(boolean)` method in [FileServiceBuilder](type).
...
By default, [FileService](type) caches up to 1024 files whose length is less than or equal to
65,536 bytes. You can customize this behavior using [FileServiceBuilder](type).
...
[FileService](type) can be configured to serve a pre-compressed file based on the value of the
...
[FileService](type) could look for `/index.html.gz` first and send the following response with the
...
This behavior is enabled by calling [FileServiceBuilder#serveCompressedFiles(boolean)](type) with `true`.
...
If you set [FileServiceBuilder#autoDecompress(boolean)](type) to `true`, the [FileService](type) automatically
...
You can also serve an individual file using [HttpFile](type). Like [FileService](type) does, [HttpFile](type)
...
An [HttpFile](type) can be configured to send different headers than the auto-filled ones using
[HttpFileBuilder](type). For example, you can:
...
## Caching `HttpFile`
Unlike [FileService](type), [HttpFile](type) does not cache the file content.
Use [HttpFile#ofCached(HttpFile,int)](type) to enable content caching for an existing [HttpFile](type):
...
An [HttpFile](type) usually does not store its content in memory but reads its content on demand, allowing you
to stream a potentially very large file. If you want to ensure the content of the file is kept in memory so
that file I/O does not occur on each retrieval, you can use the [HttpFile#aggregate(Executor)](type) method:
...
Note that an aggregated [HttpFile](type) is not linked in any way from the [HttpFile](type) it was aggregated
from, which means the content and attributes of the aggregated [HttpFile](type) does not change when the original
[HttpFile](type) changes. Use [HttpFile#ofCached(HttpFile,int)](type) instead if such behavior is necessary.
...
`byte[]` or `String`. Use [HttpFile#of(HttpData)](type) or [HttpFile#builder(HttpData)](type) to build an
[AggregatedHttpFile](type) from an in-memory resource:
