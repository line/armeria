    ...
    public HttpResponse hello(ServiceRequestContext ctx) {
        ...
        ctx.blockingTaskExecutor().execute(() -> { // 👈
            try {
                ...
                future.complete(HttpResponse.of(500);
            }
        });
...
By default, Armeria uses a context-aware event loop as a [coroutine dispatcher](https://kotlin.github.io/kotlinx.coroutines/kotlinx-coroutines-core/kotlinx.coroutines/-coroutine-dispatcher/).
A context-aware blocking executor is used as a dispatcher if you annotate a service method with <type://@Blocking> or enable <type://AnnotatedServiceBindingBuilder#useBlockingTaskExecutor(boolean)>.
