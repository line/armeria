...
- [Generated Java code](/tutorials/grpc/blog/create-service#3-compile-the-proto-file)
- `BlogService.java`
- `Main.java`
...
@Order(7)
void deleteBlogPost() {
  final DeleteBlogPostRequest reqeust = DeleteBlogPostRequest.newBuilder()
                                                             .setId(1)
                                                             .build();
  client.deleteBlogPost(reqeust);
  ...
  assertThat(exception).extracting("status.code").isEqualTo(Code.NOT_FOUND);
  assertThat(exception).hasMessageContaining("The blog post does not exist. ID: 1");
}
```
...
@Test
@Order(7)
void deleteInvalidBlogPost() throws JsonProcessingException {
  final Throwable exception = catchThrowable(() -> {
    client.deleteBlogPost(DeleteBlogPostRequest.newBuilder().setId(100).build());
  });
  assertThat(exception).isInstanceOf(StatusRuntimeException.class);
  assertThat(exception).extracting("status.code").isEqualTo(Code.NOT_FOUND);
  assertThat(exception).hasMessageContaining("The blog post does not exist. ID: 100");
}
```
...
4. (Optional) To access the Documentation service result easily, edit the log message in the `main()` method.
  ...
  ```
5. Now, [re-run the server](/tutorials/grpc/blog/run-server#2-run-the-server).
