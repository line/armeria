...
- The request now completes even when the content sanitizer throw an exception. #4248
...
- <type://EurekaUpdatingListener> now registers itself with the `instanceId` which is `hostname:appname:port`,
  so it doesn't cause a conflict when more than one app are running in the same host. #4223
