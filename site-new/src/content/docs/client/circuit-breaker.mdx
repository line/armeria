...
You can align the circuit breaker with the client's [SuccessFunction](type) by composing a rule with
[CircuitBreakerRule#builder()](type):
...
`onSuccessFunctionResult(true)` matches when the [SuccessFunction](type) regards the response as a
success, so the rule reports it as a circuit breaker success before `myRule` is consulted.
