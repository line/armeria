...
`CompletionStage[HttpResponse]` when converting an asynchronous result into an [HttpResponse](type):
...
- Conversion to [HttpResponse](type)
...
You can use the `eventLoopExecutionContext` extension method in [RequestContext](type) to specify a Scala
