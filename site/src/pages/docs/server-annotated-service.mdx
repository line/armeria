...
In order to use the blocking task executor, you can
[inject](/docs/server-annotated-service#other-classes-automatically-injected) the
<type://ServiceRequestContext> and call <type://ServiceRequestContext#blockingTaskExecutor()>:
...
Or, you can just use the <type://Blocking> annotation:
...
public class MyAnnotatedService {
    ...
    @Get("/hello")
    public HttpResponse hello(ServiceRequestContext ctx) {
        // A blocking task executor executes this method.
        assert !ctx.eventLoop().inEventLoop();
...
You can apply the <type://Blocking> annotation to the class level so all methods are executed by the
