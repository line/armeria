...
- **GraalVM Support**: Users can now easily build [GraalVM](https://www.graalvm.org/) native images for Armeria applications. #5005
- **Micrometer Observation Support**: Support for [Micrometer Observation](https://micrometer.io/docs/observation) is added.
  ...
  ...
  ```
- **Websocket Client**: You may now use <type://WebSocketClient> to call websocket services. #4972
  ```java
  ...
  ```
- **Implement gRPC Richer Error Model more easily**: <type://GoogleGrpcStatusFunction> is introduced so that users
  can more easily implement gRPC [Richer Error Model](https://grpc.io/docs/guides/error/#richer-error-model). #4614 #4986
  ```java
  ...
  ```
- **New API for Multipart Headers**: Users can now retrieve headers from a multipart request in an annotated service
...
- **Create a StreamMessage from Future**: You can now easily create a <type://StreamMessage> from a future
