...
...
                  .runtimeWiring(c -> {
                      ...
                      DataFetcher<String> fileUpload = environment -> {
                          MultipartFile multipartFile = environment.getArgument("file");
                          ...
                      }
                      c.type("Mutation", typeWiring -> typeWiring.dataFetcher("fileUpload", fileUpload));
                  })
...
```
...
recorded when Armeria's server receives the first header of a request. #4365 #4372
