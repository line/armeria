...
1. Create an ID generator for temporary post IDs and a map for containing blogs posts. Add in a logger for debugging.
...
This time, let's add a proper method for creating a blog post. Previously, we made a call to create a blog post, from the client's `main()` method for testing. Leave the code inside the `main()` for testing purpose.
