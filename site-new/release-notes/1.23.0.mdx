...
- [DocService](type) now supports auto-completion for gRPC and Thrift services. #4516
...
- Unhandled exceptions are logged periodically out of the box. #4324 #4687
  - You can change the interval of the periodic logging
    using [ServerBuilder#unhandledExceptionsReportIntervalMillis(long)](type).
  ```java
  ServerBuilder sb = ...
...
- You can now disconnect connections gracefully
  using [ClientRequestContext#initiateConnectionShutdown()](type). #4555 #4708
  ```java
  ClientRequestContext ctx = ...
  // The connection will be gracefully shut down after a request is finished.
  ctx.initiateConnectionShutdown();
  ```
- You can now use [ServiceErrorHandler](type) to configure the per-service or per-virtual host
...
- You can now handle an HTTP/1 request with an absolute URI
  using [ServerBuilder#absoluteUriTransformer(Function)](type). #4681 #4726
  ```java
  ServerBuilder sb = ...
  ...
  ```
- You can now listen to the result of DNS queries using [DnsQueryListener](type). #4690 #4715
  ```java
  DnsAddressEndpointGroup
    ...
    .build();
  ```
- You can now create a [BlockingTaskExecutor](type) by wrapping a `ScheduledExecutorService`. #4760
  - Additionally, you can create context-aware or context-propagating `Executor` and
    [BlockingTaskExecutor](type).
  ```java
  ...
  ```
- You can now use [StreamMessage#of(InputStream)](type) to convert an `InputStream`
  to [StreamMessage](type). #3937 #4703
  ```java
  InputStream in = ...
  // The input stream is divided into chunks of 8192 bytes by default.
  ByteStreamMessage stream = StreamMessage.of(in);
  ```
- You can now use [StreamMessage#streaming()](type) to create [StreamWriter](type) for writing
  ...
  writer.subscribe(...);
  ```
- You can now generate a [RequestId](type) using the information of [RoutingContext](type). #4362 #4691
...
- You can now collect metrics of open and closed client connections
  using [ConnectionPoolListener#metricCollecting(MeterRegistry)](type). #4685 #4686
  ```java
  ...
  ClientFactory factory = ClientFactory.builder()
                                       ...
                                       .build();
  ```
- [ClientRequestContext#authority()](type) can now be used to get the authority that will be sent to
  the server. #4697
  ```java
  ClientRequestContext ctx = ...
...
- You can now execute gRPC `ServerInterceptor` asynchronously in Kotlin
  using [CoroutineServerInterceptor](type). #4669 #4724
  ```kotlin
...
- The debug page of [DocService](type) is now a pop-up for easier access. #4599
- You can now use `Connection: close` header to close a connection after sending or receiving a response.
  #4131 #4454 #4471 #4531
- Armeria now exports the gauge metrics of [CommonPools#workerGroup()](type). #4675 #4750
  - The metric names:
...
- The [RetryConfig](type) from [RetryConfigMapping](type) is retrieved only once for a retry session
...
- `ConcurrentModificationException` is not raised anymore when using [AsyncServerInterceptor](type). #4729
- [RequestHeaders#path()](type) now returns percent-encoded value like
  [RequestContext#path()](type) does when used in an Armeria server. #4694 #4721
- [ByteStreamMessage#range(long,long)](type) of a path stream message now handles
  the overflow of the range correctly. #4705
- The authority header is now correctly set for retried requests. #4697
- You no longer see [DnsTimeoutException](type) when an IPv6 network interface is enabled. #4695
- No more deadlocks occur in Thrift 0.14 or older when loading [DocService](type). #4688
- [HttpStatusException](type) is not raised anymore when there is no matching route for a request. #4548 #4552
...
- A server-side [RequestLog](type) is completed correctly when an empty response is sent. #4807

## 🏚️ Deprecations

- [ClientFactory#setMeterRegistry(MeterRegistry)](type) is now deprecated in favor of
  [FlagsProvider#meterRegistry()](type). #4785

## ☢️ Breaking changes

- Spring Boot 1 integration is no longer supported. #4651 #4787
- The type of `blockingTaskExecutor` property such as [ServiceConfig#blockingTaskExecutor()](type) is
  changed from `ScheduledExecutorService` to [BlockingTaskExecutor](type). #4760
  - Simply recompiling your code should be enough in most cases because [BlockingTaskExecutor](type)
    is a `ScheduledExecutorService`.
- The return types of `makeContextAware` methods on [RequestContext](type) are changed to the
  context-aware types. #4760
  - `makeContextAware(Executor)` returns [ContextAwareExecutor](type) instead of `Executor`.
  - `makeContextAware(ScheduledExecutorService)` returns [ContextAwareScheduledExecutorService](type)
    instead of `ScheduledExecutorService`.
- The signatures of `UserClient.execute()` have been changed. #4789
  - It now contains the newly added [RequestTarget](type) as a parameter instead of the `path`, `query`,
    and `fragment`.
