...
You can create a [WebClient](type) with a base URI and send a request with a relative path.
...
You can also create a [WebClient](type) without a base URI and send a request with an absolute URI.
The ad-hoc [WebClient](type) would be useful in the following cases:
...
You can enrich your [WebClient](type) by decorating it with decorators.
...
- [BraveClient](type) which [traces](/docs/advanced/zipkin) outbound requests using 
  [Brave](https://github.com/openzipkin/brave).
- [CircuitBreakerClient](type) which [opens a circuit](/docs/client/circuit-breaker) on failed responses.
- [CookieClient](type) which stores cookies of responses using [CookieJar](type).
- [ConcurrencyLimitingClient](type) which limits the concurrent number of active requests.
- [ContentPreviewingClient](type) which
  [previews the content](/docs/advanced/structured-logging/#enabling-content-previews) of
  requests and responses.
- [DecodingClient](type) which decompresses a
  [compressed](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Encoding) response.
- [MetricCollectingClient](type) which 
  [collects metrics](/docs/advanced/metrics#collecting-client-side-metrics) using 
  [Micrometer](https://micrometer.io/).
- [LoggingClient](type) which logs requests and responses.
- [RetryingClient](type) which [automatically retries](/docs/client/retry) failed requests.
...
mechanisms with [EndpointGroup](type), 
...
Requests are sent to dynamically retrieved [Endpoint](typeplural) by specifying an [EndpointGroup](type)
to a [WebClient](type).
...
You can build your own [ClientFactory](type) using [ClientFactoryBuilder](type) and 
set it through [WebClientBuilder#factory()](type).
