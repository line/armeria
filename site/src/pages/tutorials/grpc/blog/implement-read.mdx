...
In the prior step, we tried to create a blog post. In this step, we'll implement a read operation and make a call
to read blog posts. We'll write two service methods, one for reading a single post and another for multiple posts.

One major difference to the earlier step is the client instance used. Instead of using the client instance we created, we'll create another one. Armeria offers various options to create client instances.
...
This time, we'll use a different client instance. Instead of using the client instance created with <type://Clients>, we'll use <type://GrpcClients>.

1. Add another `BlogServiceBlockingStub` in the `BlogClient` class with a different name.
  ...
  ```
2. Add a new client instance using `GrpcClients`.
  ```java filename=BlogClient.java
  class BlogClient {
  ...
  ```
3. Add a method each for retrieving posts. Note to use `cb`, the client instance created with <type://GrpcClients>.
...
Retrieving a single blog post requires a post ID. We haven't saved any post ID returned by the server. (Although we very much know that our tutorial service issues incremental ID.). To check IDs, we'll use Armeria's <type://LoggingClient> to decorate the our blog client. Likewise, you can decorate your service also. To decorate your service, use the [server decorator](https://armeria.dev/docs/server-decorator) or annotate your gRPC stub class with <type://@LoggingClient>.

<Tip>

  Another option available is interceptor. See <type://GrpcServiceBuilder#intercept()> for information.

</Tip>

1. Add a decorator to the client instance, `cb`.
  ```java filename=BlogClient.java
