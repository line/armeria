...
In this step, we go back to the [Step 1. Create a server](./tutorial-blog-rest-step1) and add the blog service to the server. Also, we add [Armeria's DocService](/docs/server-docservice) to test our blog service.
...
In [Step 1. Create a server](./tutorial-blog-rest-step1), you've added a dummy service just to see a server launched. Now, remove the dummy service and add our blog service.

1. From Main.java, remove the dummy service by removing lines 4 and 5 as shown below.
  ...
  static Server newServer(int port) {
    ...
    return sb.http(port)
              ...
              .build();
  ```
2. Add our service by adding line 4 as shown below.
...
To test and call our service operations, use Armeria's <type://DocService>.
...
1. Click the URL `http://127.0.0.1:8080/docs` from the log message or open the URL on a web browser. If you see the <type://DocService> page, you have launched the services and server successfully.
  ...
  static Server newServer(int port) {
    ...
    final DocService docService =
            DocService.builder()
              ...
              .build();
  ```
3. Click the **SUBMIT** button, and you will see the blog post information returned in the right panel.
