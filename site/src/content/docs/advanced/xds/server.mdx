...
and services you register via `ServerBuilder`. xDS never generates, translates,
or modifies your routing or service definitions. It manages the layers above:
TLS and HTTP filters.
...
xDS decorators are outermost (run first), so that policies like RBAC reject
before user decorators or services see the request:

```
[xDS jwt_authn] → [xDS rbac] → [user's service decorators] → service
```
...
skip filters on a health check path by disabling the filter for that route:
        ...
        - match: { prefix: "/healthcheck" }
          ...
          typed_per_filter_config:
            envoy.filters.http.rbac:
              "@type": type.googleapis.com/my.custom.FilterConfig
              disabled: true
        - match: { prefix: "/" }
          non_forwarding_action: {}
```
...
requests to upstream clusters — it serves them directly:
...
Server server = Server.builder()
                      .plugin(XdsServerPlugin.of(xdsBootstrap, "server-listener"))
                      .service("/hello", (ctx, req) -> HttpResponse.of("hello"))
                      .build();
server.start().join();
```

The server blocks during startup until the first xDS snapshot arrives (default
timeout: 30 seconds), so it never serves with stale or missing configuration.
