...
To test updating a blog post, you need to have blog posts created, which requires you to have the creation method implemented in the BlogService.java. See [Step 3. Implement CREATE](/tutorials/tutorial-blog-rest-step3) for instructions.
...
3. Create a couple of blog posts to test updating a blog post and get the ID value returned. Enter the cURL commands below.
...
4. Let's update the second blog with the ID, `1`. Change the value for the `"title"` and `"content"` properties.
  ...
  ```bash
  {"id":1,"title":"Update title to this title","content":"Update blog content with this content.","createdAt":1624864060343,"modifiedAt":1624869413163}
  ```

5. Let's retrieve the same blog post to see if the change has been applied successfully.
