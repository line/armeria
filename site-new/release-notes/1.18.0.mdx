...
- [DocService](type) can now render Markdown and [Mermaid](https://mermaid-js.github.io/mermaid/)
...
- You can now bind decorators to each gRPC service using [GrpcServiceBuilder#addService(BindableService,Iterable)](type). #4311 #4316
...
- [UnframedGrpcErrorHandler#ofJson()](type) now conforms to
...
- You can now extract the innermost element of [Unwrappable](type) type objects easily using [Unwrappable#unwrapAll()](type). #4338
...
- [RequestContext](type) now extends [Unwrappable](type). #4308

## 🛠️ Bug fixes

- You no longer see an intermittent [IllegalStateException](type) for gRPC services
  when [GrpcServiceBuilder#useBlockingTaskExecutor(boolean)](type) is enabled. #4367 #4374
- Requests no longer hang when gRPC-JSON transcoding is enabled and gRPC decorators return an unframed response. #4337 #4348
- You can now use [HttpFile](type) to serve files from the OSGI bundle. #4371 #4373
...
- You no longer see a `NoSuchElementException` while a [HealthCheckedEndpointGroup](type) is closed. #4349
- [ClosedSessionException](type) and [ClosedStreamException](type) are raised correctly according to their usage. #4380
