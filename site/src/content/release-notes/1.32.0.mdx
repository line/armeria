...
- **Load Balancer**: You can now create a [LoadBalancer](type) using various load balancing strategies to
...
- **Filter Invalid IP address**: You can now reject invalid IP addresses using
  [ClientFactoryBuilder#ipAddressFilter(Predicate)](type). #6106 #6111
  ```java
  ClientFactory
    .builder()
    .ipAddressFilter(ip -> { // 👈👈👈
  ...
  ```
- **Graceful Shutdown**: You can now use [GracefulShutdown](type) to terminate unfinished requests
...
- **Map Injection for an Annotated Service**: The annotated service now supports the `Map<String, Object>`
  ...
  ```
- **Eureka InstanceInfo**: You can now retrieve a Eureka [InstanceInfo](type) from an [Endpoint](type)
  via [InstanceInfo#instanceInfo(Endpoint)](type). #6056 #6069
  ```java
  ...
  ```
- **Client Preprocessor**: You can now use a client [Preprocessor](type) to customize certain properties. #6057
  ```java
  ...
  WebClient client =
    WebClient.builder()
             ...
             .build();
  ```
- **Backoff Builder**: You can now easily create a [Backoff](type) using builder. #5483 #5488
  ```java
  Backoff backoff =
...
- You can now see the remoteAddress in the error message when a [SessionProtocolNegotiationException](type)
  is raised. #6113
- You can now use the default [HealthCheckUpdateHandler](type) via [HealthCheckUpdateHandler#of()](type) #6002

## 🛠️ Bug fixes

- A cookie is correctly overwritten when a cookie with the same name, domain, and path is set. #5870 #6126
- [CorsPolicyBuilder#build()](type) is exposed publicly again. #6121
- Connections do not hold references on automatically aborted requests, resulting in less memory pressure. #6108
- [CircuitBreakerRuleBuilder#onTimeoutException()](type) and [RetryRuleBuilder#onTimeoutException()](type)
...
- You no longer see a false positive warning log when [HealthCheckedEndpointGroup](type) is closing. #6063
- An [Endpoint](type) no longer includes attributes in `.toString()`. #6061
- Fixed a bug where a trailing dot was included in the hostname used by SNI. #6044 #6046
- Log levels for responses are now correctly mapped based on the [SuccessFunction](type). #6042
...
- You can now sample the stacktrace of [ResponseCompleteException](type) with `verboseExceptionSampler`. #5972
- The [ProxyConfig](type) now correctly respect the TTL of a DNS record. #5843 #5960
- A recovered [HttpResponse](type) by the [ServiceErrorHandler](type) now correctly produces a response
  content preview. #3969 #5076

## 🏚️ Deprecations

- [ServerConfig#gracefulShutdownQuietPeriod()](type) and [ServerConfig#gracefulShutdownTimeout()](type)
  are now deprecated. #5941
  - Use [ServerConfig.gracefulShutdown()](type) instead.

## ☢️ Breaking changes

- The `json_name` field option will only be used when [HttpJsonTranscodingQueryParamMatchRule#JSON_NAME](type)
  is explicitly specified via [HttpJsonTranscodingOptionsBuilder#queryParamMatchRules(Iterable)](type). #6136
