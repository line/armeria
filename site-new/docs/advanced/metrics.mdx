...
You can use [MetricCollectingService](type) with [MeterIdPrefixFunction](type) for collecting metrics
of your services.
...
for a gRPC service, you can use [GrpcMeterIdPrefixFunction](type) instead of [MeterIdPrefixFunction](type).
...
You can provide [GrpcMeterIdPrefixFunction](type) as a bean if you are using a Spring integration module:
...
you can use the [ServerBuilder#meterRegistry(MeterRegistry)](type) method like this:
...
This approach can be used to collect metrics for a [WebClient](type) and a gRPC client:
...
provide a custom meter registry. To accomplish this, override the [ClientFactory](type) in this way:
...
you can set your own `DistributionStatisticConfig` using [MoreMeters#setDistributionStatisticConfig(DistributionStatisticConfig)](type).
...
Please refer to [MeterIdPrefixFunction](type) to learn what kinds of tags are used for request metrics.
