...
As you might have noticed already, we decorated the client using [LoggingClient](type), which logs all
requests and responses. You might be interested in decorating a client using other decorators, for example
to gather metrics. Please also refer to [ClientBuilder](type) for more configuration options.

## Retrying failed requests

Use [RetryingClient](type) as a decorator to automatically retry failed gRPC requests.

:::note

gRPC clients use HTTP/2 under the hood, so you should use [RetryingClient](type) (HTTP-level),
not [RetryingRpcClient](type).

:::

For simple cases, [RetryRule.failsafe()](type) retries on connection errors and unprocessed requests:
...
Because [RetryingClient](type) operates at the HTTP layer, it cannot see gRPC-level exceptions such as
`StatusRuntimeException`. Those are thrown by the stub only after the HTTP response has been fully
received and processed — after the retry decision point. Use `onException()` only for HTTP-layer
exceptions like [ResponseTimeoutException](type) or [UnprocessedRequestException](type).

To retry based on gRPC status codes, inspect the gRPC trailers instead.

:::

To retry based on gRPC status codes, use [RetryRuleWithContent](type) with
`onGrpcTrailers()`:
...
RetryRuleWithContent<HttpResponse> retryRule =
    RetryRuleWithContent.<HttpResponse>builder()
                        .onGrpcTrailers((ctx, trailers) -> {
                            int grpcStatus = trailers.getInt(GrpcHeaderNames.GRPC_STATUS, -1);
                            return grpcStatus == Status.UNAVAILABLE.getCode().value() ||
...
Please refer to [Automatic retry](/docs/client/retry) for more information, including how to configure
`maxTotalAttempts`, per-attempt timeouts, backoff strategies, and integration with circuit breakers.
...
in the server will be returned to the client as a [StatusCauseException](type) attached to the normal gRPC
...
- [Automatic retry](/docs/client/retry)
