...
/**
 * The result returned by {@link PathMapping#apply(RoutingContext)}.
 */
final class PathMappingResult {

    private static final PathMappingResult EMPTY =
            new PathMappingResult(null, null, ImmutableMap.of());

    /**
     * The {@link PathMappingResult} whose {@link #isPresent()} returns {@code false}. It is returned by
     * {@link PathMapping#apply(RoutingContext)} when the specified path did not match.
     */
    public static PathMappingResult empty() {
        return EMPTY;
    }

    static PathMappingResultBuilder builder() {
        return new PathMappingResultBuilder();
    }
    ...
    PathMappingResult(@Nullable String path, @Nullable String query, Map<String, String> pathParams) {
