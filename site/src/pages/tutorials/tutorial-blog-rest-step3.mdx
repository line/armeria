...
2. Declare a class, `BlogService`, in the file as shown below. Import required files as you go along the tutorial or copy all from the [sample file](https://github.com/line/armeria-examples/blob/master/rest-api-annotated-service/src/main/java/example/armeria/server/blog/BlogService.java).
...
1. Create a BlogPostRequestConverter.java file.
2. Declare a class in the file, implementing the <type://RequestConverterFunction> interface. For simplicity sake, generate impromptu IDs in this tutorial.
  ...
  ```
3. Add a method retrieving a value of a given key in a JSON object:
  ...
  ```
4. Customize the default `convertRequest()` method as follows.
    - Map the `title` and `content` properties of the body parameter to members in a `BlogPost` object, using the `stringValue()` method we wrote in the previous step.
    - Throw the <type://FallthroughException> by calling <type://RequestConverterFunction#fallthrough()>, to pass on the request parameters that aren't converted by our converter to the default request converter.
    - Feel free to add more mapping blocks (lines 9 to 11) in this request converter for other requests for the blog service.
    ```java filename=BlogRequestConverter.java highlight=9,10,13
    @Override
...
In this step, assign the [request converter we customized](#write-a-request-converter) to our service method. Annotate the service method with <type://@RequestConverter> and specify the RequestConverterFunction class as `BlogPostRequestConverter.class`.
...
When the request for creation is received, our request converter creates an instance of a blog post object for us. We want to save the blog post object in the map (`blogPosts`) created in the `BlogService` class. Skip adding a database operation for this tutorial.

To store the blog post information in the map, add the following line in the `createBlogPost()` method.
...
1. Specify JSON object keys for each member of our blog post object, by annotating the following methods with `@JsonProperty`.
  ...
  ```
2. Create and return an HTTP response using Armeria's <type://HttpResponse> and include the information of the post created, as shown below, in the `createBlogPost()` method.
  ```java filename=BlogService.java
  public HttpResponse createBlogPost(BlogPost blogPost) throws JsonProcessingException {
    ...
