...
- **Configure EventLoopGroup for services.**: You can now set an `EventLoopGroup` for a server,
  a virtual host and a service to run a service with it.
  #4099 #5233
  ```java
  final EventLoopGroup serviceWorkerGroup =
    EventLoopGroups.newEventLoopGroup(8, "service-worker");
  Server.builder()
        .route()
        ...
        .serviceWorkerGroup(serviceWorkerGroup)
        .service(myService) // myService is run with serviceEventLoopGroup.
        ...
        .build();
  ```
- **Context Path Support**: You can now set services/decorators under a context path fluently. #3591 #4802 #5037 #5157
  ```java
  Server.builder()
        .contextPath("/v1", "/v2")
...
- **gRPC exception handler Annotation**: You can now easily handle an exception that is raised from a gRPC service using
  ...
  private static class MyGrpcExceptionHandlerFunction implements GrpcExceptionHandlerFunction {
    @Override
    public Status apply(RequestContext ctx, Throwable cause, Metadata metadata) {
      if (unauthenticatedExcpetion(cause)) {
  ...
  ```
- **StreamMessage finalizer**: You can now produce the last value for a <type://StreamMessage> when it completes exceptionally using
  <type://StreamMessage#endWith(Function)>. #4816 #5198
  ```java
...
- You can now add a hook that is invoked when a <type://RequestContext> is pushed and popped. #4991 #5107
  ...
  Server
    ...
    .build();
  ```
- **Notify CircuitBreak on Duration Exceed**: You can now report a failure to a circuit breaking when the total duration exceeds a certain threshold.
  #4782 #4827
  ```java
  final CircuitBreakerRuleWithContent<HttpResponse> rule =
        CircuitBreakerRuleWithContent
                ...
                .thenFailure();
  ```
- **RST Floods Attack Prevention**: You can now protect your server against
  [DDoS caused by RST floods](https://www.securityweek.com/rapid-reset-zero-day-exploited-to-launch-largest-ddos-attacks-in-history/)
...
- A recovered <type://HttpResponse> by <type://ServiceErrorHandler> produces a response content preview. #3969 #5076
- The `Content-Length` header isn't set anymore for the streaming response of a HEAD request. #4509 #4644

## 🏚️ Deprecations

- <type://GrpcStatusFunction> is deprecated. Use <type://GrpcExceptionHandlerFunction> instead. #5046
...
- Netty incubator 0.0.21.Final → 0.0.23.Final
