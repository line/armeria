...
- You can now create a [HealthChecker](type) that updates its healthiness periodically. #3392 #3406
  ```java
  HealthChecker.of(() -> {
  ...
  ```
- You can now create a propagating [ContextAwareExecutorService](type) and
  [ContextAwareScheduledExecutorService](type) using [RequestContext#makeContextPropagating(ExecutorService)?full](type)
  and [RequestContext#makeContextPropagating(ScheduledExecutorService)?full](type).
  The executors propagate a [RequestContext](type) to submitted tasks when a submission thread is 
...
- You can now set a maximum allowed length of a server response in the request level
  using [WebClientRequestPreparation#maxResponseLength(long)](type). #3565
  ```java
  WebClient client = ...;
  client.prepare()
...
- You can now add or set HTTP headers and query params with `java.util.Map`. #3499 #3502
  ...
  QueryParams.builder()
             ...
             .set(Map.of("size", "2"));
  ```
- You can now disable DNS query metrics using [DnsResolverGroupBuilder#disableDnsQueryMetrics()](type).
  #3368 #3552
  ```java
  ...
  ```
- You can now create a [JSON Lines](https://jsonlines.org/) response using [JsonLines](type). #3446 #3456
  ```java
  ...
  ```
- You can now create an [RpcRequest](type) with a service name using
  [RpcRequest#of(Class,String,String,Iterable)](type). #3537
- You can now customize [MetricCollectingService](type) with [MetricCollectingServiceConfigurator](type)
...
- You no longer see an infinity loop or an `IllegalStateException: Recurse update` when invalidating 
  a DNS cache. #3528
- [LoggingService](type) now correctly logs a request failed with the server error class(5xx) that is served 
  by [TransientService](type). #3551 #3559
