...
- **Context Path Support**: You can now set services/decorators under a context path fluently. #3591 #4802 #5037 #5157 #5260
  ...
  armeria:
    context-path: /api/v1
  ```
- **Easier Request Scoping for Kotlin**: You can now easily propagate an Armeria [RequestContext](type) in Kotlin using
  [ContextAwareExecutor#asCoroutineDispatcher()](type). #5171
  ```kotlin
...
- **gRPC Exception Handler Annotation**: You can now easily handle an exception that is raised from a gRPC service using
  [@GrpcExceptionHandler](type) annotation. #5046
  ```java
  ...
  ```
- **StreamMessage finalizer**: You can now produce the last value for a [StreamMessage](type) when it completes
  exceptionally or successfully using [StreamMessage#endWith(Function)](type). #4816 #5198
  ```java
...
- You can now add a hook that is invoked when a [RequestContext](type) is pushed and popped. #4991 #5107
  ```java
...
- **RST Flood Attack Prevention**: You can now protect your server against
  ...
  using [ServerBuilder#http2MaxResetFramesPerWindow(int,int)](type). #5232
...
- The `CompletableFuture`s of a [RequestLog](type) are now completed with the
  [RequestContext#eventLoop()](type) thus their callbacks are also executed by the `EventLoop`. #5189
- Adding services and decorators are now unified. #5038 #5040

## 🛠️ Bug fixes

- [RequestLog#responseCause()](type) is correctly propagated to [CircuitBreakerRule](type) when using
  a [CircuitBreakerClient](type) with a [RetryingClient](type). #5242
...
- An `IllegalStateException` is raised if [GrpcSerializationFormats#JSON](type) isn't set when
  [GrpcServiceBuilder#enableHttpJsonTranscoding(boolean)](type) is enabled. #5176 #5224
- [CookieClient](type) is now correctly working with redirects. #5221
- You no longer see duplicate [ClientOptions](type) when creating a derived client via
  [Clients#newDerivedClient(T,Function)](type). #5194
- You no longer see `UnsupportedOperationException` when creating an [HttpResponse](type) with
...
- You no longer see a false negative [EndpointSelectionTimeoutException](type) when
  [EndpointSelectionStrategy#weightedRoundRobin()](type) is used with a [DynamicEndpointGroup](type). #5163
...
- Armeria gRPC service no longer schedules a timeout if `grpc-timeout` header is omitted when
  [GrpcServiceBuilder#useClientTimeoutHeader(boolean)](type) is enabled. #5144
- The `Content-Length` header isn't set anymore for the streaming response of a HEAD request. #4509 #4644

## 🏚️ Deprecations

- [GrpcStatusFunction](type) is deprecated. Use [GrpcExceptionHandlerFunction](type) instead. #5046 #5267
  - `exceptionMapping` methods on [GrpcServiceBuilder](type) are deprecated.
    Use [GrpcExceptionHandlerFunctionBuilder](type) instead.
  - [GoogleGrpcStatusFunction](type) is also deprecated. Use [GoogleGrpcExceptionHandlerFunction](type)
    instead.
