...
With [Server-Sent Events][Server-Sent Events], however, it is possible for a server to push a new data to
the web page whenever it wants to.

Armeria provides several factory methods by <type://ServerSentEvents> class which help a user to easily send
an event stream to the client. The following example shows how to build a server which serves services
sending a response with [Server-Sent Events][Server-Sent Events].
...
Of course, Armeria provides <type://@ProducesEventStream> annotation in order to convert the result objects
returned from an annotated service method into [Server-Sent Events][Server-Sent Events].
...
As you might know, it is not only for [Server-Sent Events][Server-Sent Events], so you can use this method for
