...
- The metrics of requests to a [TransientService](type) are not collected anymore by default. #3061 #3081
  - Access logs and service logs are not recorded as well.
  - You should use [TransientServiceOption](type) to enable them.
    ```java
    HealthCheckService.builder()
                      ...
                      .build();
    ```
  - Currently, [HealthCheckService](type) and [PrometheusExpositionService](type) are
    [TransientService](typeplural).
...
- You can now use Scala `Future` in an annotated service. #3189
  ...
  ```
- You can now use [HttpDeframer](type) to conveniently decode a stream of [HttpObject](typeplural)
  to N objects. #2981
  ```java
  ...
  - See [HttpDeframer](type) for more information.
- You can now apply [CircuitBreaker](type) per request path. #3134, #3135
  ```java
  ...
  ```
- You can now apply the different `maxTotalAttempts` and `responseTimeout` for [RetryingClient](type)
  using [RetryConfig](type). #3145
  ```java
  ...
  ```
- You can now split the [ResponseHeaders](type) and bodies using [HttpResponse#split()](type). #3038
...
- You can now specify a prefix for MDC keys using the `<prefix>` element. #3086 #3112
  ...
  ```
- You can now use the unsafe TLS cipher using [ClientFactoryBuilder#tlsAllowUnsafeCiphers(boolean)](type).
  #3157 #3172
- You can now specify an arbitrary type for [@Header](type) and [@Param](type) if the
...
- You can now build and execute an [HttpRequest](type) fluently. #3110
  ```java
  ...
  client.prepare()
        ...
        .execute();
  ```
- You can now easily handle cookies by applying [CookieClient#newDecorator()](type). #2637 #3118
  ```java
  ...
  ```
- You can now use the custom Thrift protocol by using [ThriftProtocolFactoryProvider](type) and SPI. #3183
  ```java
...
- You can now customize the [HealthCheckService](type) when using Spring integration. #3144
  ```java
  @Bean
  ...
  ```
- You can now use [RequestHeaders#acceptLanguage()](type) to choose language. #3177 #3179

{/* truncate */}

## 📈 Improvements

- Various improvements for [DocService](type). #3149 #3150 #3167 #3188

## 🛠️ Bug fixes

- [HealthCheckedEndpointGroup#endpoints()](type) now returns healthy endpoints properly even when
  [EndpointGroup#orElse(EndpointGroup)](type) is used. #3181
- `ServletRequest.getProtocol()` now returns the proper value when using [TomcatService](type) and
  [JettyService](type). #3194
...
- You no longer see `AnnotatedConnectException` when the [Endpoint](type) is created with an
  IPv6 scope ID. #3158 #3178
- Armeria server does not reject the request path whose first segment includes a colon anymore. #3154

## 🏚️ Deprecations

- [CircuitBreakerClient#newPerHostAndMethodDecorator(BiFunction,CircuitBreakerRule)](type) is now deprecated.
  #3135
  - Use [CircuitBreakerClient#newDecorator(CircuitBreakerMapping,CircuitBreakerRule)](type) with the customized
    [CircuitBreakerMapping](type) using [CircuitBreakerMapping#builder()](type).
- The response timeout and max total attempts setters in [RetryingClientBuilder](type) are now deprecated.
  #3128 #3145
  - The static factory methods that take those parameters in [RetryingClient](type) are now deprecated as well.
  - Use [RetryConfigMapping](type) and [RetryConfig](type).
- [Route#apply(RoutingContext)?full](type) is deprecated. #3152
  - Use [Route#apply(RoutingContext,boolean)?full](type).
- The constructor of [PrometheusExpositionService](type) is now deprecated. #3081
  - Use [PrometheusExpositionService#of(CollectorRegistry)](type).
