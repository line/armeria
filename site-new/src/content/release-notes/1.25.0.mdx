...
- **Micrometer Observation Support**: Support for [Micrometer Observation](https://docs.micrometer.io/micrometer/reference/observation) is added.
  Refer to [ObservationClient](type) or [ObservationService](type) for details on how to integrate with Armeria.  #4659 #4980
  ```java
...
- **WebSocket Client Support**: You can now send and receive data over [WebSocket](https://en.wikipedia.org/wiki/WebSocket)
  using [WebSocketClient](type). #4972
  ```java
...
- **Implement gRPC Richer Error Model More Easily**: You can now easily use gRPC
  [Richer Error Model](https://grpc.io/docs/guides/error/#richer-error-model) via [GoogleGrpcStatusFunction](type). #4614 #4986
  ```java
...
- **Set HTTP Trailers Easily**: You can now easily set trailers to be sent after the data stream using
  [HttpRequest#of(RequestHeaders,Publisher,HttpHeaders)](type) or
  [HttpResponse#of(ResponseHeaders,Publisher,HttpHeaders)](type). #3959 #4727
- **New API for Multipart Headers**: You can now retrieve headers from a multipart request in an annotated service
  using [MultipartFile#headers()](type). #5106
- **Access RequestLogProperty Values More Easily**: [RequestLogAccess#getIfAvailable()](type)
  has been introduced, which allows users to access a [RequestLogProperty](type) immediately if available. #4956 #4966
- **Keep an Idle Connection Alive on PING**: The `keepAliveOnPing` option has been introduced. Enabling this option will keep
  ...
  by [ServerBuilder#idleTimeout(Duration,boolean)](type) or [ClientFactoryBuilder#idleTimeout(Duration,boolean)](type). #4794 #4806
- **Create a StreamMessage from Future**: You can now easily create a [StreamMessage](type) from a `CompletionStage`
  using [StreamMessage#of(CompletionStage\<?\>)](type). #4995
- **More Shortcuts for PrometheusExpositionService**: You can now create a [PrometheusExpositionService](type) without
...
- You can now customize error responses when a service for a request is not found
  using [ServiceErrorHandler#renderStatus()](type). #4996
- Redirection for a trailing slash is done correctly even if a reverse proxy rewrites the path. #4994
- [DocService](type) now tries to guess the correct route behind a reverse proxy. #4987
- The `RetentionPolicy` of [@UnstableApi](type) annotation is now `CLASS` so that
  bytecode analysis tools can detect the declaration and usage of unstable APIs. #5131

## 🛠️ Bug fixes

- [GrpcService](type) now returns an `INTERNAL` error code if an error occurs while serializing gRPC metadata. #4625 #4686
- [DnsCache](type) now allows zero TTL for resolved DNS records. #5119
...
- [ServiceRequestContext#localAddress()](type) and [ServiceRequestContext#remoteAddress()](type) now return
...
- You can now use either an inline debug form or a modal debug form when using [DocService](type). #5072
...
- [AggregatedHttpObject#content(Charset)](type) now respects the `charset` attribute in the
...
- Documentation on how [Flags](type) work in Armeria has been added. #4870
- A new example on how to use [krotoDC](https://github.com/mscheong01/krotoDC) with Armeria has been added. #5092

## ☢️ Breaking changes

- The `toStringHelper()` method in [DynamicEndpointGroup](type) has been replaced
  with `toString(Consumer)` to avoid exposing an internal API in the public API. #5132

## 🏚️ Deprecations

- [HttpResponse#from(CompletionStage)](type) and its variants methods are deprecated. #5075
  - Use [HttpResponse#of(CompletionStage)](type) and its variants instead.
