...
public final class HedgingRpcClient extends AbstractHedgingClient<RpcRequest, RpcResponse>
        implements RpcClient {

    public static Function<? super RpcClient, HedgingRpcClient> newDecorator(HedgingConfig<RpcResponse> config) {
        requireNonNull(config, "config");
        return newDecorator((ctx, req) -> config);
    }

    public static Function<? super RpcClient, HedgingRpcClient> newDecorator(HedgingConfigMapping<RpcResponse> mapping) {
        requireNonNull(mapping, "mapping");
        return delegate -> new HedgingRpcClient(delegate, mapping);
    }

    HedgingRpcClient(RpcClient delegate, HedgingConfigMapping<RpcResponse> mapping) {
    ...
    private void doExecute0(ClientRequestContext ctx, RpcRequest req,
        ...
        final HedgingConfig<RpcResponse> hedgeConfig = mappedHedgingConfig(ctx);
        final HedgingRuleWithContent<RpcResponse> hedgingRule =
                hedgeConfig.needsContentInRule() ?
                hedgeConfig.hedgingRuleWithContent() : hedgeConfig.fromHedgingRule();
        res.handle((unused1, cause) -> {
            try {
                assert hedgingRule != null;
                hedgingRule.shouldHedge(derivedCtx, res, cause).handle((decision, unused3) -> {
//                    final Backoff backoff = decision != null ? decision.backoff() : null;
//                    if (backoff != null) {
//                        final long nextDelay = getNextDelay(derivedCtx, backoff);
//                        if (nextDelay < 0) {
//                            onHedgeComplete(ctx, derivedCtx, res, future);
//                            return null;
//                        }
//
...
//                    return null;
    ...
    private static void onHedgeComplete(ClientRequestContext ctx, ClientRequestContext derivedCtx,
                                        RpcResponse res, CompletableFuture<RpcResponse> future) {
        onHedgingComplete(ctx);
