...
## Using [@Part](type) annotation

While [@Param](type) treats multipart parts as simple strings or files,
the [@Part](type) annotation uses the `Content-Type` header of each part
...
|---|---|
| `File`, `Path`, [MultipartFile](type) | File upload (same as [@Param](type)) |
...
[@Part](type) also supports `List` and `Set` for receiving multiple parts with the same name:
