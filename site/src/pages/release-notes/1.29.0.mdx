...
- You can now easily find both dynamic and static decorators that handle a request
  ...
  ```
- You can now use the marshaller specified by gRPC `MethodDescriptor`. #5103 #5630
  ```java
...
- You can now programmatically retrieve the server side metrics via <type://ServerMetrics>. #4992 #5627
  ...
  ```
- You can now specify a [Custom methods](https://cloud.google.com/apis/design/custom_methods) on HTTP endpoint. #5613
  ```java
  ServerBuilder builder = ...
  builder.service("/v1/some/resource/name:customVerb", customService);
  ```
- You can now use a <type://CoroutineHttpService> that runs your service in a coroutine scope. #5442 #5603
  ...
  ```
- You can now specify options for an <type://HttpService> via <type://HttpServiceOptions>. #5071 #5574
  ```java
  ...
  class MyService {
      @HttpServiceOption(requestTimeoutMillis = 10000, maxRequestLength = 1024)
      @Get("/hello")
...
- You can now inject a custom attribute from a <type://ServiceRequestContext> to an annotated service
  ...
  class MyAnnotatedService {
    ...
    @Get("/hello")
    public HttpResponse hello(
      @Attribute(prefix = MyAttributes.class, value = "USERNAME") String username) { ... }
  }
  ```
- You can now specify a graceful shutdown timeout for a <type://ClientFactory>. #5470 #5489
  ```java
  ClientFactory
    .builder()
...
- You can now specify an <type://EndpointGroup> to build a <type://ClientRequestContext>. #5292 #5298
  ...
  ClientRequestContext ctx =
    ClientRequestContext.builder(request)
...
- An <type://XdsEndpointGroup> now supports
  [Slow start mode](https://www.envoyproxy.io/docs/envoy/latest/intro/arch_overview/upstream/load_balancing/slow_start). #5688 #5693
- Priority and locality based load balancing is now available for an <type://XdsEndpointGroup>. #5610
- An Armeria client now correctly handles `Expect: 100-continue` header. #5394 #5646
...
- <type://HealthCheckedEndpointGroup> no longer raises an <type://EndpointSelectionTimeoutException> when
  a server is stopped by the JVM shutdown hook. #5718
...
- You can now specify a field for gRPC transcoding with `p{\d}` pattern. #5679
...
- A `DecompressionException` isn't ignored anymore in a <type://StreamDecoder>. #5177 #5620
...
- <type://PrometheusMeterRegistries> and <type://PrometheusExpositionService> are deprecated. #5698
  - Use the same classes in the `armeria-promehteus1` module.
...
- Micrometer 1.13.0 uses Prometheus Java client 1.x. #5698
  ...
  - If you want to use Prometheus Java client 1.x, add `com.linecorp.armeria:armeria-promehteus1` module.
