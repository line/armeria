...
Once you've finished the implementation of the service, you need to build a [GrpcService](type) using
a [GrpcServiceBuilder](type) and add it to the [ServerBuilder](type):
...
We bound the [GrpcService](type) without specifying any path. It is because [GrpcService](type)
implements [HttpServiceWithRoutes](type), which dynamically provides [Route](typeplural) by itself.

:::

:::tip

[GrpcService](type) supports [gRPC][gRPC], [gRPC-Web][gRPC-Web] and [Protobuf-JSON][Protobuf-JSON] by default.
If you don't want your [GrpcService](type) support all those formats, specify the formats you want
using [GrpcServiceBuilder#supportedSerializationFormats(Iterable)](type).

:::

## Decorating a `GrpcService`

Unlike a usual Armeria service, [GrpcService](type) implements a special interface called
[HttpServiceWithRoutes](type). Therefore, it is recommended to decorate a [GrpcService](type) by specifying
...
about [HttpServiceWithRoutes](type).

:::

The above decorator decorates all services. If you want to decorate only a specific service, you need to specify
the decorators when you add the service to the [GrpcServiceBuilder](type),
or use the [@Decorator](type).
...
[GrpcService](type) supports the [gRPC-Web][gRPC-Web] protocol,
...
in terms of CORS. Armeria provides [CorsService](type) to handle this requests, so you need to decorate it when
you build a [GrpcService](type):
...
[GrpcService](type) supports serving unary RPC methods (no streaming request or response) without [gRPC][gRPC]
wire format framing. This can be useful for gradually migrating an existing HTTP POST based API to [gRPC][gRPC].
As [GrpcService](type) supports both binary protobuf and [Protobuf-JSON][Protobuf-JSON], either legacy protobuf
...
annotate the gRPC services or methods with a [@Blocking](type) annotation, wrap the entire service implementation in
`ServiceRequestContext.current().blockingTaskExecutor().submit()`, or set
[GrpcServiceBuilder#useBlockingTaskExecutor(boolean)](type) so the above happens automatically for
all service methods and lifecycle callbacks.

### Using [@Blocking](type) annotation
...
### Wrapping a service with [ServiceRequestContext#blockingTaskExecutor()](type)
...
### Using [GrpcServiceBuilder#useBlockingTaskExecutor(boolean)](type)
...
It can be very useful to enable [Flags#verboseResponses()](type) in your server by specifying the
