...
[RequestContext](type) of the request being handled. [RequestContextExportingAppender](type) is
a Logback appender that exports the properties of the current [RequestContext](type) to
...
There are three types of properties you can export using [RequestContextExportingAppender](type).

## Built-in properties

A built-in property is a common property available for most requests. See the complete list of the built-in
properties and their MDC keys at [BuiltInProperty](type).
...
A user can attach an arbitrary custom attribute to a [RequestContext](type) by using
[RequestContext custom attributes](/docs/advanced/logging/#custom-attributes) to store the information associated with the
request being handled.
[RequestContextExportingAppender](type) can export such attributes to the [MDC][MDC] property map as well.
...
By default, [RequestContextExportingAppender](type) uses `Object.toString()` to convert an attribute value
