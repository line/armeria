...
You can also use <type://HttpHeaders> to set a complex headers into <type://BobyPart#headers()>.
...
`Path`, `File`, or <type://MultipartFile> using <type://@Param> annotation.

<Tip>

Note that a <type://BodyPart> can be converted into a `Path`, `File` or <type://MultipartFile> only when 
a <type://BodyPart#filename()> is specified.
