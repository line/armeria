...
more types able to be returned from the annotated service.

- :api:`HttpResponse` and :api:`AggregatedHttpMessage`

  - It will be responded to the client without any modification. Exception handlers will be invoked
    if an exception is raised while the response is being sent. If no message has been sent to the client yet,
...
- ``CompletionStage<?>``
  ...
  - An object which is generated by the ``CompletionStage<?>`` will be converted to an :api:`HttpResponse`
    by response converters. If the ``CompletionStage<?>`` fails, exception handlers will handle the cause.

- Other types

  - Other types will be converted to an :api:`HttpResponse` by response converters. If a service method
    raises an exception, exception handlers will handle it.
