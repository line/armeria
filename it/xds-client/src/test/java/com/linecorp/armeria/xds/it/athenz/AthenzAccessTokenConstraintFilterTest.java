...
import com.linecorp.armeria.xds.it.athenz.AthenzAccessTokenFilterFactory.AthenzInboundXdsHttpFilter;
...
/**
 * Tests the inbound {@code AccessTokenConstraint} filter end-to-end.
 *
 * <p>The client-side outbound filter (via xDS) injects a Bearer token, and the
 * server-side inbound filter validates it using {@link AthenzAuthorizer}.
 */
...
class AthenzAccessTokenConstraintFilterTest {
    ...
    @RegisterExtension
    @Order(3)
    static final ServerExtension protectedServer = new ServerExtension() {
        @Override
        protected void configure(ServerBuilder sb) {
            sb.service("/files", (ctx, req) -> HttpResponse.of("ok"));
        }
    };
        ...
        reconfigureServer("""
                ...
                assertion_mapping:
                  ...
                  - action:
                      literal: obtain
                    resource:
                      literal: "testing:files"
                """);
        // Send without a token directly to the protected server.
        final BlockingWebClient directClient = protectedServer.blockingWebClient();
        final AggregatedHttpResponse response = directClient.get("/files");
    ...
    void validTokenWithLiteralMapping() {
                ...
                assertion_mapping:
                  ...
                  - action:
                    ...
                    resource:
                      literal: "testing:files"
                """);
        pushXdsConfig(outboundListener());
        ...
        try (XdsBootstrap xdsBootstrap = XdsBootstrap.of(bootstrap);
            ...
            // Client gets token via outbound xDS filter → server validates via inbound filter.
    ...
    void validTokenWithTemplateMapping() {
        // Use a regex capture on the path to strip the leading '/' so the resource
        // matches the Athenz policy "testing:files" (not "testing:/files").
        reconfigureServer("""
                constraint_domain: testing
                syntax_version: 1
                assertion_mapping:
                  ...
                  - conditions:
                    ...
                    resource:
                      template:
                        template: "testing:${match.pathCapture.1}"
                """);
        pushXdsConfig(outboundListener());
    ...
    private static void reconfigureServer(String constraintYaml) {
        final AccessTokenConstraint constraint =
                XdsResourceReader.fromYaml(constraintYaml, AccessTokenConstraint.class);
        final AthenzAuthorizer authorizer =
                AthenzAuthorizer.builder(athenz.newZtsBaseClient(AthenzDocker.TEST_SERVICE))
                                .policyConfig(new AthenzPolicyConfig(AthenzDocker.TEST_DOMAIN_NAME))
                                .build();
        final DecoratingHttpServiceFunction decorator =
                new AthenzInboundXdsHttpFilter(authorizer, constraint).serviceDecorator();
        protectedServer.server().reconfigure(sb -> {
    ...
    private void pushXdsConfig(Listener listenerResource) {
        ...
        final ClusterLoadAssignment assignment = XdsResourceReader.fromYaml("""
                ...
                """.formatted(CLUSTER_NAME,
                              protectedServer.httpSocketAddress().getHostString(),
                              protectedServer.httpPort()), ClusterLoadAssignment.class);
    ...
    private static Listener outboundListener() {
