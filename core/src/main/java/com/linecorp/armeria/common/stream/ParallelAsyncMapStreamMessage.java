...
final class ParallelAsyncMapStreamMessage<T, U> implements StreamMessage<U> {
    private final StreamMessage<T> source;
    private final Function<T, CompletableFuture<U>> function;

    @SuppressWarnings("unchecked")
    ParallelAsyncMapStreamMessage(StreamMessage<? extends T> source,
    ...
    public void subscribe(Subscriber<? super U> subscriber, EventExecutor executor,
        ...
        source.subscribe(new AsyncMapParallelSubscriber<>(subscriber, function, executor), executor, options);
    }
    ...
    private static final class AsyncMapParallelSubscriber<T, U> implements Subscriber<T>, Subscription {
        ...
        private long incompleteFutures;
        private boolean completing;

        AsyncMapParallelSubscriber(Subscriber<? super U> downstream,
        ...
        private void publishDownstream(@Nullable U item, @Nullable Throwable cause) {
            ...
            try {
                if (cause != null) {
                    ...
                    if (completing && incompleteFutures == 0) {
