...
consult it, which can lead to a subtle pitfall: if such a response is classified as
...
The simplest is the [CircuitBreakerClientBuilder](type) flag, which composes a [SuccessFunction](type)
check before your rule:
...
For finer-grained control, compose [CircuitBreakerRule#onSuccessFunctionMatch()](type) yourself:

```java
final CircuitBreakerRule rule =
        CircuitBreakerRule.onSuccessFunctionMatch()
                          .orElse(myRule);
```
