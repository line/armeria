...
- New [CircuitBreakerRule](type) and [CircuitBreakerRuleWithContent](type) API let you define a complex
  ...
  ```
- You can now easily bind an MDC property to a [RequestContext](type) and log it using a logging framework.
  #2634 #2724 #2729
  ```java
  ...
  ```
- You can now install a custom hook to [RequestContextStorage](type) in runtime, usually at startup time. #2723
  ```java
...
- You can now set method parameter descriptions for annotated services in
  [DocService](type) using Javadoc. #2697
- You can now easily sanitize headers and content using [RegexBasedSanitizer](type). #1810 #2721

{/* truncate */}

## 📈 Improvements

- You can now easily call `ownAttr()` from [RequestContext](type). #2727
  - You don't have to check if the context is [ClientRequestContext](type) or not anymore.
...
- You can now retry even when the current [EndpointGroup](type) is empty. #2736
- You can send a request by setting the authority and scheme header to [RequestHeaders](type). #2730 #2734
- You no longer see `&lt;zero-length response&gt;` in [DocService](type) anymore. #2732
...
- `RetryStrategy` and `RetryStrategyWithContent` are gone in favor of
  [RetryRule](type) and [RetryRuleWithContent](type). #2726
- `CircuitBreakerStrategy` and `CircuitBreakerStrategyWithContent` are gone in favor of
  [CircuitBreakerRule](type) and [CircuitBreakerRuleWithContent](type). #2726
- `EndpointSelector.select()` (and thus EndpointGroup.select() as well) now returns `null`
  instead of throwing an [EmptyEndpointGroupException](type). #2736
