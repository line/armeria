...
- You can now bind [GraphQL Java](https://www.graphql-java.com/) with <type://ServerBuilder> 
  using <type://GraphqlService>. #3318 #3373
  ```java
  ServerBuilder sb = ...;
  ...
  `armeria-graphql` is currently experimental, so you should be careful using the feature.
...
- You can now transform the objects of <type://HttpRequest> and <type://HttpResponse> using a function.
  ...
  ```java
  HttpRequest req1 = ...;
  HttpRequest req2 = req1.mapHeaders(headers -> ...)
                         .mapData(data -> ...)
                         .mapTrailers(trailers -> ...);
  
  HttpResponse res1 = ...;
  HttpResponse res2 = res1.mapHeaders(headers -> ...)
...
- You can now collect the all elements of a <type://StreamMessage> with <type://StreamMessage#collect()>. #3603
  ```java
  StreamMessage<MyObject> stream = ...;
  CompletableFuture<List<MyObject>> collected = stream.collect();
  ```
- You can exclude a <type://Route> from another <type://Route> when configuring their services. #2737 #3562
  ```java
  ServerBuilder sb = ...;
  sb.route()
    .pathPrefix("/api")
    .exclude("/api/admin")
    .exclude(Route.builder()...build())
    .build(apiService);
...
- You can now use <type://RequestOptions> to configure various options at request level. #3593
  ...
  // Specify the options when building a request using WebClientRequestPreparation.
  client.prepare()
        .get("/some/path")
        .responseTimeout(Duration.ofSeconds(20))
        .writeTimeout(Duration.ofSeconds(5))
...
- You can now conveniently configure a loopback port with
  ...
  ```
- You can now specify the default Jackson modules with <type://JacksonModuleProvider>. #3594 #3625
  - All available Jackson modules are automatically loaded through Jackson's
    [built-in SPI](https://fasterxml.github.io/jackson-databind/javadoc/2.11/com/fasterxml/jackson/databind/cfg/MapperBuilder.html#findAndAddModules--)
    by default.
- [TCP User Timeout Option](https://datatracker.ietf.org/doc/html/rfc5482) is enabled by default when an idle
  timeout is enabled. #3509
  - `TCP_KEEPIDLE` and `TCP_KEEPINTVL` options are enabled when a PING interval is greater than 0.
- You can now get the last value of a header name or a query param using
  <type://HttpHeaders#getLast(CharSequence)> and <type://QueryParams#getLast(String)>. #3438 #3568
  ```java
  HttpHeaders headers = ...;
  headers.getLast(HttpHeaderNames.X_FORWARDED_FOR);
  
  QueryParams params = ...;
  params.getLast("items");
  ```
- You can now specify a current and max content length when creating <type://ContentTooLargeException>.
  #3520 #3616
...
- You can now figure out how many connections it is managing using <type://ClientFactory#numConnections()>.
  #3596 #3613
- You can now specify the number of event loop threads and blocking task threads using a number. #3597 #3602
  ```java
  Server.builder()
        .workerGroup(3)
...
- `TemporaryThreadLocals` throws an `IllegalStateException` if it was not properly released before using it.
  #3505
- You can now use optimized a context-aware `CompletableFuture` for Java 12 and above. #2119 #3615
- Various multipart mime types are added to <type://MediaType>. #3642

## 🛠️ Bug fixes

- `ByteBuf`s are no longer leaked when using:
  - Spring WebFlux integration with compression #3608 #3617
...
- `CancellationScheduler` provides more accurate timeout nanoseconds. #3570
