...
A [CircuitBreaker](type) can be one of the following three states:
...
- `OPEN`
  - The state machine enters the `OPEN` state once the number of failures divided by the total number of
    requests exceeds a certain threshold. All requests are blocked off responding with [FailFastException](type).
- `HALF_OPEN`.
...
[Request](type) and [Response](type) types:

- [CircuitBreakerClient](type)
- [CircuitBreakerRpcClient](type)

Let's use [CircuitBreakerClient](type) to find out what we can do.
You can use the `decorator()` method in [WebClientBuilder](type) to build a [CircuitBreakerClient](type):
...
Now, the [WebClient](type) can track the number of success or failure events depending on the
[Response](typeplural). The [CircuitBreaker](type) will enter `OPEN`, when the number of failures divided
by the total number of [Request](typeplural) exceeds the failure rate.
Then the [WebClient](type) will immediately get [FailFastException](type) by the [CircuitBreaker](type).

## `CircuitBreakerRule`

How does a [CircuitBreaker](type) know whether a [Response](type) is successful or not?
[CircuitBreakerRule](type) does the job. In the example above, if the status of a [Response](type) is `5xx`
or an `Exception` is raised during the call, the count of failure is increased.
You can have your own `rule` by building [CircuitBreakerRule](type).
The following example builds a [CircuitBreakerRule](type) that fails when an `Exception`
...
you should build [CircuitBreakerRuleWithContent](type) and specify it when you create an
[WebClient](type) using [CircuitBreakerClientBuilder](type):
...
## Aligning with `SuccessFunction`

[ClientOptions#SUCCESS_FUNCTION](type) lets you classify certain responses (e.g., HTTP `4xx`, business
exceptions) as success for metrics and logging. By default a [CircuitBreakerRule](type) does not
consult it.

:::warning

If a response that the [SuccessFunction](type) regards as a success is classified as
`CircuitBreakerDecision.ignore()` by your rule, an `OPEN` [CircuitBreaker](type) will never close,
because every trial request in `HALF_OPEN` keeps getting ignored.

:::

There are two ways to align the circuit breaker with the client's [SuccessFunction](type).

The simplest is the [CircuitBreakerClientBuilder](type) flag, which consults the
[SuccessFunction](type) before your rule:
...
WebClient client = WebClient
        .builder(...)
        .successFunction((ctx, log) -> log.responseHeaders().status().code() == 404)
        .decorator(CircuitBreakerClient.builder(myRule)
...
- If the [SuccessFunction](type) regards the response as a success, it is reported as a success and
  your rule is skipped.
- Otherwise your rule runs as usual.

For finer-grained control, build the rule yourself using [CircuitBreakerRule#builder()](type):
...
`onSuccessFunctionResult(boolean)` matches when the [SuccessFunction](type) returns the given value,
so it can be combined with any terminal (`thenSuccess()`, `thenFailure()`, `thenIgnore()`) to express
either direction.

## Grouping `CircuitBreaker`s

In the very first example above, a single [CircuitBreaker](type) was used to track the events. However,
in many cases, you will want to use different [CircuitBreaker](type) for different endpoints. For example, there
...
Having one [CircuitBreaker](type) that tracks all the success and failure does not make sense in this scenario.
It's even worse if the client connects to the services on different machines.
When one of the remote services is down, your [CircuitBreaker](type) will probably be `OPEN` state although
...
- Group by host: a single [CircuitBreaker](type) is used for each remote host.
...
- Group by method: a single [CircuitBreaker](type) is used for each method.
...
- Group by host and method: a single [CircuitBreaker](type) is used for each method and host combination.
...
We have used static factory methods in [CircuitBreaker](type) interface to create a [CircuitBreaker](type) so far.
If you use [CircuitBreakerBuilder](type), you can configure the parameters which decide
[CircuitBreaker](type)'s behavior. Here are the parameters:

- `name`:
  - The name of the [CircuitBreaker](type).
- `failureRateThreshold`:
  - The threshold that changes [CircuitBreaker](type)'s state to `OPEN` when the number of failed
    [Request](typeplural) divided by the number of total [Request](typeplural) exceeds it.
    The default value is `0.5`.
- `minimumRequestThreshold`:
  - The minimum number of [Request](typeplural) to detect failures. The default value is `10`.
- `trialRequestInterval`:
  - The duration that a [CircuitBreaker](type) remains in `HALF_OPEN` state. All requests are blocked off
    responding with [FailFastException](type) during this state. The default value is `3` seconds.
- `circuitOpenWindow`:
...
- `counterSlidingWindow`:
  - The duration of a sliding window that a [CircuitBreaker](type) counts successful and failed
    [Request](typeplural) in it. The default value is `20` seconds.
- `counterUpdateInterval`:
  - The duration that a [CircuitBreaker](type) stores events in a bucket. The default value is `1` second.
- `listeners`:
  - The listeners which are notified by a [CircuitBreaker](type) when an event occurs. The events are one of
    `stateChanged`, `eventCountUpdated` and `requestRejected`. You can use
    [CircuitBreakerListener#metricCollecting()](type) to export metrics:
...
[CircuitBreaker](type) API is designed to be framework-agnostic and thus can be used with any non-Armeria
clients:

1. Create a [CircuitBreaker](type).
2. Guard your client calls with `CircuitBreaker.tryRequest()`.
3. Update the state of [CircuitBreaker](type) by calling [CircuitBreaker#onSuccess()](type) or
   [CircuitBreaker#onFailure()](type) depending on the outcome of the client call.
