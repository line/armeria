...
Serve the embedded tomcat service via Armeria using <type://ArmeriaServerConfigurator> bean.
...
Even if you don't use Spring MVC based on web servlets like Tomcat, there is still value in using Spring and Armeria together.
You can leverage Spring Boot's automatic configuration and dependency injection to make the Armeria application.
...
Armeria's annotated service is similar to Spring controller. Spring will inject required dependency of the annotated service.
You can make the annotated service as a bean and inject it into the server building process.
For example:
...
Inject this annotated service to the <type://ArmeriaServerConfigurator> bean.
...
The following configuration prevents the execution of Spring's embedded web server and instead runs the Armeria.
