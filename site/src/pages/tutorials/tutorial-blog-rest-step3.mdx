...
Now that we've prepared minimal files for the tutorial in [step 2](/tutorials/tutorial-blog-rest-step2), it's time to start writing the core of the service.

<TutorialSteps current={3} />

In this step, you'll create a file to contain service methods. The first service method to write is for creating a blog post. The HTTP method to map with is POST.

By completing this step, you'll learn to map your service with the HTTP POST (<type://@Post>) method and make your own request converter (<type://@RequestConverter>).

## What you need

You need the BlogPost.java file, created from [step 2](/tutorials/tutorial-blog-rest-step2). You can always [download](https://github.com/line/armeria-examples/blob/master/tutorials/rest-api-annotated-service/src/main/java/example/armeria/server/blog/BlogPost.java) the full version of this file, instead of creating one yourself.

## 0. Create a service file

Create a service file, BlogService.java. Import required files as you go along the tutorial or copy at once from the [sample file](https://github.com/line/armeria-examples/blob/master/tutorials/rest-api-annotated-service/src/main/java/example/armeria/server/blog/BlogService.java).
...
Let's start with [mapping the HTTP POST method](/docs/server-annotated-service#mapping-http-service-methods) with our service method:

1. Declare a service method, `createBlogPost()`, in the class `BlogService`.
2. Map this service method with the HTTP POST method by adding the <type://@Post> annotations.
...
To handle parameters received for creating a blog post, use <type://RequestConverter>, in these steps. The order may be changed.
...
```java filename=BlogService.java highlight=5
@Post("/blogs")
@RequestConverter(BlogPostRequestConverter.class)
public HttpResponse createBlogPost(BlogPost blogPost)
    throws JsonProcessingException {
...
Let's return a response for blog post creation. Create and return an HTTP response using Armeria's <type://HttpResponse> and include the information of the post created in the `createBlogPost()` method.
  ```java filename=BlogService.java
  public HttpResponse createBlogPost(BlogPost blogPost)
      throws JsonProcessingException {
    ...
    return HttpResponse.of(HttpStatus.OK, MediaType.JSON_UTF_8,
                              mapper.writeValueAsBytes(blogPost));
  }
  ```
