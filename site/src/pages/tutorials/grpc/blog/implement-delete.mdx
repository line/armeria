...
Let's implement a service method 'deleteBlogPost()` in the BlogService.java we've been working on.
...
This time, let's use the first client instance we created, `client`. Add a method `deleteBlogPost()` to send a request to update a blog post.
...
4. Now, while keeping the server running, run the client again, attempting to delete the post with the ID 0, which does not exist any longer.
  ...
  ```
   Suppose we have no time and just want to know if an error has occurred. One way we can check is using the interceptor, again. So, change the client instance from `client` to `cb` in the `deleteBlogPost()` method and run the client again.
...
Here, we've implemented a service method and client method for deleting a blog post. Next, we'll [add](/tutorials/grpc/blog/add-docservice) Armeria's [Documentation service](/docs/server-docservice) to our service.
