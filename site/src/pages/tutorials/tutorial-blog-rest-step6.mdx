...
In this step, we will write a method for deleting a blog post. The HTTP method we map with this method is DELETE.

<TutorialSteps current={6} />

By completing this step, you will learn to map your service with HTTP DELETE (<type://@Delete>), customize an exception handler, and use a blocking task executor.
...
Start by [mapping HTTP DELETE method](/docs/server-annotated-service#mapping-http-service-methods) with our service method:
...
3. [Map this service method](/docs/server-annotated-service#mapping-http-service-methods) with the HTTP DELETE by adding the <type://@Delete> annotation.
...
For deleting a blog post, take the blog post ID (`id`) to identify the post to delete, as a path parameter.
...
To delete a blog post, copy the following code inside the `deleteBlogPost()` method.
...
2. Create an exception handler for the blog service:
  ...
  ```
3. Implement our own exception handler by overriding the default `handleException()` method. Add a code block for handling the `IllegalArgumentException` thrown. For our tutorial, return a BAD REQUEST as the response.
...
With real services, accessing and operating on a database takes time. To prevent the service to wait on our task, we want to inform the task executor to handle other tasks until we are done with database operation.

To enable this, we use Armeria's [blocking task executor](/docs/server-annotated-service#specifying-a-blocking-task-executor). There are a few options of implementing this; we will annotate our service method with the <type://@Blocking>.
...
We've already handled returning the not found error in the [exception handling section](#handle-exception). Here, we will return a response for successful deletion.
...
Next, at [Step 7. Add blog service to server](./tutorial-blog-rest-step7), let's go back to the main method and add the blog service to our server.
